...
pair supports four options: [Filesystem](/docs/pm-tools/filesystem) (no PM tool account — backlog in your repo; a `code-host` is still declared), [GitHub Projects](/docs/pm-tools/github-projects) (free, great for teams on GitHub), [Linear](/docs/pm-tools/linear) (dedicated PM tool with a polished UI), and [Azure DevOps](/docs/pm-tools/azure-devops) (Microsoft ecosystem; hosts the code too, so no separate `code-host`). You can switch between them at any time by updating `way-of-working.md`.
...
Yes. pair-cli works with any directory containing AGENTS.md and a `config.json`. Create a Git repository with your skills, write a config.json that defines the registries and targets, and distribute it with `pair-cli install --source <git-url>`. No Knowledge Base required.
