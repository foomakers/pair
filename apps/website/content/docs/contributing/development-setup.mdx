...
equals the OUTPUT of the real `pair-cli update` transform — never the dataset source itself, which the
...
working tree's own dataset, offline; `pair-cli update` installs the latest PUBLISHED knowledge base and
...
| Pre-push fails on a mirror guard | Run `pnpm mirrors:regenerate` and commit the regenerated files — never hand-edit a mirror. |
