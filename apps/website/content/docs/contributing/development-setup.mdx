...
the next diff. On a `format:check` failure, run `pnpm format` and commit the result —
`gate:composition` guards the gate against a write-mode step creeping back in. See ADL
