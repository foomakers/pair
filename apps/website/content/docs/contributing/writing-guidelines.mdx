...
| `ddr-template.md` | Domain Decision Record |
| `analysis-log-template.md` | Analysis-Log entry (category `Analysis`, shares `decision-log/` with ADL) |
...
# Build the website to verify content renders (through turbo, so the
# workspace packages it type-checks against are built first)
