...
The catalog guard in `packages/knowledge-hub/src/tools/claude-plugin-manifest.test.ts` (run by `pnpm test`) fails when the manifest drifts from `packages/knowledge-hub/dataset/.skills/`, naming the exact entry to add or remove — so a forgotten update is caught before release, not after.
