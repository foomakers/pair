...
The [marketplace channel](/docs/integrations/claude-code#install-channels) is served by two hand-maintained files at the repository root: `.claude-plugin/marketplace.json` and `.claude-plugin/plugin.json`. They are deliberately **not generated** — when the skill catalog changes (a skill is added, renamed, or removed), update them in the same PR:
