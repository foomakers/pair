...
- Skills only — `plugin.json` carries metadata plus `skills` and nothing else; the marketplace entry carries metadata plus `source`/`category`/`tags`/`strict` and **no `skills`** (an entry-level `skills` silently replaces the plugin's whole catalog); and no root-level `agents/`, `commands/`, `hooks/`, `skills/` or `.mcp.json`: the plugin source is `"./"`, so the repository itself is the payload and Claude Code auto-discovers those components at its root whatever the manifest says.

The catalog guard in `packages/knowledge-hub/src/tools/claude-plugin-manifest.test.ts` (run by `pnpm test`) fails when the manifest drifts from `packages/knowledge-hub/dataset/.skills/`, naming the exact entry to add or remove — so a forgotten update is caught before release, not after. The same guard asserts the skills-only rule as an **allowlist** of permitted manifest keys — so `settings`, `dependencies`, `monitors`, `commands` and any key a future schema revision adds fail closed too — plus the five root-level component paths, so that checklist line has an automatic backstop too.
