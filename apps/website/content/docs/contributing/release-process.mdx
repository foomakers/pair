...
- No root-level component directory named after a manifest component key (`agents/`, `commands/`, `hooks/`, `skills/`, `monitors/`, …) and no root `.mcp.json`: the plugin source is `"./"`, so the repository itself is the payload and Claude Code auto-discovers those components at its root whatever the manifest says.

The catalog guard in `packages/knowledge-hub/src/tools/claude-plugin-manifest.test.ts` (run by `pnpm test`) fails when the manifest drifts from `packages/knowledge-hub/dataset/.skills/`, naming the exact entry to add or remove — so a forgotten update is caught before release, not after. The same guard asserts the skills-only rule as an **allowlist** per surface — so `settings`, `dependencies`, `monitors`, `commands` and any key a future schema revision adds fail closed too — plus a root-payload check whose path list is *derived* from the schema's component keys, so the manifest half and the payload half of the rule can never drift apart. Both checklist lines have an automatic backstop.
