...
`flattenDepth: 2` says an entry is `<category>/<name>` deep, so anything below it (a skill's `references/` progressive-disclosure directory) installs **inside** the skill instead of beside it. Set it whenever your skills own sub-directories — see [Bounded Flattening](/docs/reference/configuration#bounded-flattening-flattendepth).
...
<Callout type="info">
  You can also host the package as a ZIP on any HTTP server, network share, or artifact registry. See [Install from URL or Path](/docs/customization/install-from-url).
</Callout>
...
- [Install from URL or Path](/docs/customization/install-from-url) — Alternative distribution methods
