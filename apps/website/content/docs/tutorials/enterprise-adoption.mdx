...
<Callout type="info">
  If using GitHub Packages with a **public** repository, consumers only need registry configuration in `.npmrc`:
  ```text
  @your-org:registry=https://npm.pkg.github.com
  ```
  No authentication token is required for installation. Publishing requires a token with `write:packages` scope.
</Callout>
