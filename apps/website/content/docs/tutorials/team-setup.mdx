...
<Callout type="info">
  If you installed pair-cli as a dev dependency instead of globally, use `npx pair-cli install` (or `pnpm pair-cli install`). See [installation options](/docs/tutorials/first-project#1-install-pair-cli) for details.
</Callout>
...
The same skill also provisions a **deterministic secret-scanning CI job** — required, at every risk tier, no LLM involved. `gitleaks` is the KB default; the job is written as a required, fail-closed step in your CI pipeline alongside a starting `.gitleaks.toml` allowlist at the project root. This is a mechanical CI gate, not a skill judgment call — see the [secret-scanning guideline](https://github.com/foomakers/pair/blob/main/packages/knowledge-hub/dataset/.pair/knowledge/guidelines/quality-assurance/security/secret-scanning.md) for the job template, the allowlist mechanism, and a reproducible verification. Security judgment (authn/authz, injection, OWASP Top 10) is a separate concern, handled per-PR by `/pair-capability-assess-security` — the two never overlap.
...
- **[Support](/docs/support)** — FAQ and troubleshooting when something doesn't work
