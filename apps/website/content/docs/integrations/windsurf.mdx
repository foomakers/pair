...
Windsurf reads project-level rules from `.windsurf/rules/` at the repository root. pair generates these bridge files during `pair-cli install`.
...
**Expected output**: Windsurf reads the rules, follows the references to `.pair/adoption/`, and describes your project context. If it can't find the rules, check that `pair-cli install` completed successfully.
