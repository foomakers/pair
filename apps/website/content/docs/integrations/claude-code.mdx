...
<Callout type="warn">
  The marketplace channel installs **skills**, not your project's adoption files. Skill steps that read `.pair/adoption/` resolve against the plugin's own copy, so a project without its own `.pair/adoption/` gets the guideline defaults (each skill's documented graceful-degradation path). Run `pair-cli init` — the CLI channel — when you want project-specific adoption files driving the skills.
</Callout>
