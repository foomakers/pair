...
The marketplace manifest lives at `.claude-plugin/marketplace.json` in the pair repository, with the plugin manifest alongside it at `.claude-plugin/plugin.json`. The plugin ships **skills only** — no agents, hooks, MCP or LSP servers, commands, monitors, settings overrides or plugin dependencies. A guard in the pair repository allowlists the keys either manifest may declare, so nothing else can be added by hand without failing the build.

Claude Code copies the plugin into its own cache (`~/.claude/plugins/cache`), so the knowledge base the skills reference travels with them and stays read-only.
