...
|------|---------|--------------|
| **Marketplace** (Claude Code native) | `/plugin marketplace add foomakers/pair` → `/plugin install pair@pair` | Installs one skill: `/pair-capability-setup-cli` |
| **Bootstrap** | `/pair-capability-setup-cli` | Installs `pair-cli` (locally when your project has a `package.json`, otherwise via `npx`), creates the git repository if there is none, then runs the CLI |
...
<Callout type="info">
  **Keeping both installed is fine here.** The plugin declares exactly one skill,
  `pair-capability-setup-cli`, and the CLI never installs that one into your project — it is
