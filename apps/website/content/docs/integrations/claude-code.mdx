...
|---------|---------|--------------|-------------|
| **Marketplace** (Claude Code native) | `/plugin marketplace add foomakers/pair` | The full skill catalog as the `pair` plugin, invoked as `/pair:pair-next` (or the bare `/pair-next`) | You work in Claude Code only and want zero setup |
...
<Callout type="warn">
  **Install one channel, not both.** Because the content is identical, installing both puts the
  same skill name in `.claude/skills/` *and* in the plugin catalog. The bare `/pair-…` form then
  stops being unique — and every cross-skill composition in the corpus (a process skill invoking
  `/pair-capability-verify-quality`, `/pair-capability-classify`, …) plus every doc reference uses
  exactly that bare form. So the cost of running both channels is not untidiness: multi-phase
  processes become ambiguous about which copy they are composing. Nothing detects this state today —
  keeping to one channel per project is on you.
</Callout>
...
The marketplace manifest lives at `.claude-plugin/marketplace.json` in the pair repository, with the plugin manifest alongside it at `.claude-plugin/plugin.json`. The plugin ships **skills only** — no agents, no hooks, no MCP servers.
...
<Callout type="warn">
  **Adoption files: the marketplace channel reads pair's, not yours.** The plugin's source is the
  ...
  `/pair-capability-verify-quality`, and any process skill composing them — require the **CLI
  channel** (`pair-cli init`), which puts your own `.pair/adoption/` in your repo. On the marketplace
  channel, stick to skills that only read the knowledge base. Project-scoped adoption resolution for
  the plugin channel is tracked in
