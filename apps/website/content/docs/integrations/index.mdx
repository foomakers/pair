...
| [opencode](/docs/integrations/opencode) | `AGENTS.md` + `.claude/skills/` | Skill discovery, `opencode run` |
| [pi](/docs/integrations/pi) | `AGENTS.md` + `.agents/skills/` | Skill discovery, `pi -p` |
...
Running from a cloud coding environment rather than a local terminal? See [Web and Cloud Environments](/docs/integrations/web-cloud-environments) — what holds there, and the one thing that does not.

### Bridges vs. Harnesses

The table above is about **skill-bridge discovery** — which file a tool reads to find your `.pair/`
directory. There is a second, separate layer: a **harness** is a tool configured to *execute* pair's
process (config locations, authentication, model provider, headless mode), and an **engine** is a
process [`pair-cli run`](/docs/reference/cli/commands#run) can drive one iteration at a time for
unattended delivery. If you are setting up automation rather than an editor, start at
[Adding a Harness](/docs/integrations/adding-a-harness) — it explains the three realizations of
fan-out and what supporting a new engine involves.
...
- [Web and Cloud Environments](/docs/integrations/web-cloud-environments) — running pair from Claude Code Web and similar cloud sessions
