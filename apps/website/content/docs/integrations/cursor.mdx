...
Cursor reads project-level rules from `.cursor/rules/` at the repository root. pair generates these bridge files during `pair-cli install`.
...
**Expected output**: Cursor reads the rules, follows the references to `.pair/adoption/`, and describes your project context (tech stack, architecture, current work items). If it can't find the rules, check that `pair-cli install` completed successfully.
