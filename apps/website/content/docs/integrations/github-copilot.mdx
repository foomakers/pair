...
GitHub Copilot in agent mode reads project instructions from `.github/` at the repository root. pair generates these bridge files during `pair-cli install`.
...
**Expected output**: Copilot reads `.github/agents/product-engineer.agent.md`, follows the references to `.pair/adoption/`, and describes your project context. If it doesn't find the agent files, check that `pair-cli install` completed successfully.
