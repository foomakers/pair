...
├── adoption/
│   ├── product/              # PRD, subdomains, contexts
│   └── tech/                 # Architecture, stack, way-of-working
...
Fastest path — let the CLI scaffold the repository (structure, `pair.config.json`,
README, `.gitignore`, and a release script):

```bash
pair-cli scaffold-kb ../acme-kb --name "Acme KB"
```

See [Scaffold an External KB](/docs/customization/external-kb).

Or create the directory structure manually:

```bash
mkdir -p .pair/knowledge/guidelines .pair/knowledge/how-to
mkdir -p .pair/adoption/product .pair/adoption/tech
```
