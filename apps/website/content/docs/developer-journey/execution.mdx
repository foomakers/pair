...
The feedback is annotation, never a gate: a locator mismatch, a body edited concurrently, or a tracker write that does not land is retried once, then reported in the comment, and the run carries on. See the [task-progress feedback guideline](/docs/reference/guidelines-catalog) for the locator, the tick-only patch rule and the fallbacks.
