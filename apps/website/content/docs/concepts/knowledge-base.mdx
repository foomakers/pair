...
### A Third Area: Working (Operational, Not Knowledge)

`.pair/working/` holds state produced *during* execution — checkpoints and reports — not knowledge or decisions. It is excluded from both registries above by design: `pair-cli install` never creates it and `pair-cli update` never touches it. Skills create `checkpoints/` and `reports/<category>/` on demand the first time they need them. See the [KB Structure Reference](/docs/reference/kb-structure) for details.
...
5. **Scaffold** — `pair-cli scaffold-kb` creates the KB as its **own repository**, versioned and released independently, so steps 1 and 3 pull from it instead of from pair's default KB. See [External KB Repository](/docs/customization/external-kb).
