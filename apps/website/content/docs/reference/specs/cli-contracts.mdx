...
export interface CliOptions {
  ...
  kb: boolean // --no-kb flag (true=download, false=offline) — INERT, see below
}
```

`kb` is parsed and carried, but nothing on the live command path reads it: its only consumers
(`validateCliOptions`, `shouldSkipKBDownload`) sit behind the KB pre-flight, which is unreachable
from the CLI today. `pair install --no-kb` installs a KB anyway — see the
