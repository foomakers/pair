...
- **Excess positionals are rejected.** Commander runs with `allowExcessArguments(false)` for all
  ...
  bind `name` to `Acme` and discard `KB`). Behavior change since `pair-cli` 0.4.3 — a wrapper or
  CI invocation carrying an extra argument (e.g. `pair install ./a ./b`) now fails; drop the extra
  argument or quote the value.
