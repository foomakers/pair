...
`manifest.json` ([foomakers/pair#395](https://github.com/foomakers/pair/issues/395)).

A slot whose `manifest.json` names a different KB than the one expected is treated as
**contaminated**: it is discarded and re-fetched rather than served, so a machine polluted
by an older CLI heals on its next install.
...
needs. There is no automatic eviction.
