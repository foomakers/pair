...
| `--source <dir>` (copied into cache)  | `~/.pair/kb/external/dir-{name}-{hash}/`    |
...
root of every slot above is `~/.pair/kb` unless `PAIR_KB_CACHE_DIR` overrides it.
...
| --------------------- | ----------------------------------- | --------------------- |
| `PAIR_KB_CACHE_DIR`   | Override the KB cache root (absolute path) | `~/.pair/kb`     |
