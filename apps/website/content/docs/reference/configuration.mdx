...
Override the default path with a top-level `working_path` field, a sibling of `asset_registries` — not nested inside it:
...
- No registry's `source`/`target` overlaps with the working area (default or overridden `working_path`) — see [Working Area Override](#working-area-override)
