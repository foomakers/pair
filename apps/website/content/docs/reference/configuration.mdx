...
`.pair/working/` (checkpoints, reports) is operational data, not knowledge — it is hard-excluded from every registry copy/mirror operation and never scaffolded by `pair install` (D14). See the [KB Structure Reference](/docs/reference/kb-structure) for the full convention.

Override the default path with a top-level `working_path` field, a sibling of `asset_registries` — not nested inside it. The value is a **project-relative** path (relative to the project root, like `asset_registries` targets):
...
When set, the override path — not `.pair/working` — is what gets excluded from registry operations, and is what `pair validate-config` checks for overlap.
...
- No registry's `target` overlaps with the working area (default or overridden `working_path`) — see [Working Area Override](#working-area-override)
