...
- Every entry must sit at that same depth. A source layout the depth cannot represent — an entry shallower than `flattenDepth` that owns a sub-directory, or one deeper than it — **aborts the copy** with an explicit error instead of installing something broken.
