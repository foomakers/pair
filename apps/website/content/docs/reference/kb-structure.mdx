...
Holds state produced **during** execution — checkpoints and reports — not knowledge. Outside every asset registry by design: `pair install` and `pair update` never create, modify, or delete anything under it.
...
Neither subdirectory is scaffolded by `pair install` — a skill creates them on demand the first time it writes a checkpoint or report. The path defaults to `.pair/working` and can be overridden with `working_path` in `config.json` (see [Configuration Reference](/docs/reference/configuration#working-area-override)); `pair validate-config` rejects any registry whose target overlaps with it, default or overridden.
