...
**Reports come in two shapes.** A **one-shot audit** is keyed by its run date (`reports/security/<YYYY-MM-DD>-audit.md`). A **panel** is a recurring, period-scoped report (`reports/cost/2026-07-cost-panel.md`) whose filename carries a **period key** — `YYYY-MM`, `YYYY-Wnn`, or `YYYY-MM-DD_YYYY-MM-DD`: re-running for the same period updates that one file **in place** (one file per period, no duplicates), a headline sits at the top with breakdowns collapsed, an empty period still produces a panel saying so, and an unwritable reports area degrades to presenting the panel inline. One convention for every periodic report — see [`working-area.md`](https://github.com/foomakers/pair/blob/main/packages/knowledge-hub/dataset/.pair/knowledge/guidelines/collaboration/working-area.md).
