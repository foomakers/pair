...
| Topic | What it covers |
| --- | --- |
| **Automation** | GitHub Actions, Azure DevOps board rules, filesystem scripts; the unattended-development eligibility policy and the tag→workflow mapping that routes a tagged card ([`tech/automation.md`](/docs/concepts/adoption-files)) |
...
| **Project Management Tools** | GitHub Projects, Linear, Jira configuration; [canonical states + state-mapping schema](/docs/concepts/canonical-states); [PR state flow (gate ≠ review)](/docs/concepts/pr-state-flow); [Definition of Ready & Done](/docs/concepts/definition-of-ready-and-done); [task-progress feedback (checklist ticks + one batched comment per run iteration)](/docs/developer-journey/execution#progress-feedback-ticks-and-one-comment) |
...
| **Delivery Metrics** | Delivery pillar metric set (bug resolution time, PR lead time, process adoption), median/p75 aggregation and the low-sample rule, the adoption-declared bug mapping, and the per-tool query mapping applied by `/pair-capability-analyze-delivery-metrics` |
...
- [Skills Catalog](/docs/reference/skills-catalog) — all 50 pair skills with commands and composition
