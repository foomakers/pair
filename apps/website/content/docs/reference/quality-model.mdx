...
The Cost pillar is assessed twice per change (refinement, then review) and monitored afterwards: `assess-cost`'s **report mode** takes a period — `YYYY-MM`, an ISO week, or an explicit range — and puts each closed PR's refinement-time *predicted* class beside the *real* class derived from its merged diff, flagging **drift** (under-predicted first: predicted green, real red). PRs classified before cost was ever assessed are kept as "no prediction — real only" rather than dropped, and the deploy-match dimension reads "not available" unless the project declares deploy/billing telemetry. The result is one **consolidated panel** in the working area (`.pair/working/reports/cost/<period-key>-cost-panel.md`), headline-first with per-PR detail collapsed, updated in place when the same period is re-run — see [KB Structure](/docs/reference/kb-structure#working--operational-area-d14) for the shared panel convention. With no cost data in the period (cost never assessed, or the `cost` tag not activated) the panel says exactly that instead of failing.
