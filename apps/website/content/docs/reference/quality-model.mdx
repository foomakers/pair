...
Optional. Absent means KB defaults apply completely — nothing fails. When present, it carries only the delta: a criticality table (service/domain → High/Medium/Low) and optional threshold overrides — see the full schema and a filled-in example in the [Guidelines Catalog](/docs/reference/guidelines-catalog#quality-assurance).
