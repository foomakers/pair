---
title: Link Update and Validation
description: How to validate, fix, and update links in your installed Knowledge Base content.
---
...
pair-cli update-link --log-level debug
```
...
pair-cli update-link --absolute --dry-run
```
...
pair-cli update-link --dry-run --log-level debug
...
- name: Install KB
  run: pair-cli install

- name: Validate links
  run: pair-cli update-link --dry-run

- name: Update links
  run: pair-cli update-link
```
