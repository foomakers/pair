...
- **Extra positional arguments are an error.** Every command rejects positionals beyond the ones
  ...
  `--name "Acme KB"`. This is a behavior change shipping in the next `pair-cli` release (0.4.3
  and earlier ignore the stray argument): a wrapper script or CI job that passed one must drop it.
...
`--name` is embedded in generated YAML, bash, and Markdown: newlines and control
characters are rejected, and the value is limited to 100 characters. Quote names that
