...
When `--interactive` is passed, the CLI enters a guided flow with smart defaults from `package.json`, `git config`, and saved preferences (`~/.pair/preferences.json`). Requires a TTY. Omitting the flag runs the same defaults in one shot (quick mode). This guided/quick duality follows the [Guided / Quick Setup Convention](/docs/contributing/writing-skills#setup-skills-guided-vs-quick).
