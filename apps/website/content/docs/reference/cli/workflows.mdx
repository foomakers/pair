...
See the [Link Update Guide](/docs/reference/cli/update-link) for comprehensive documentation.
...
pair-cli package --name "My KB" --version "1.0.0" -o dist/kb-v1.0.0.zip

# Verify package integrity before distribution
pair-cli kb-verify dist/kb-v1.0.0.zip
...
pair-cli kb-verify dist/kb-prod-v2.1.0.zip
...
pair-cli kb-verify kb-v1.0.0.zip

# If verification passes, install
pair-cli install --source kb-v1.0.0.zip
```
...
pair-cli package --interactive --name "my-kb" --version 2.0.0
```
...
pair-cli package --org --org-name "Acme Corp" --team "Platform" --compliance "SOC2,ISO27001" --distribution private

# Use org template for defaults (.pair/org-template.json)
pair-cli package --org
```
...
# View as JSON
pair-cli kb-info dist/kb-v1.0.0.zip --json
```

**Common Scenarios:**

- **Pre-distribution check**: Always run `pair-cli kb-verify` before publishing packages
...
- **Package inspection**: Use `pair-cli kb-info <path>` to view package metadata without extracting
...
# Package KB for distribution
pair-cli package -o dist/kb.zip
...
See the [Troubleshooting](/docs/support/troubleshooting) for detailed solutions.
