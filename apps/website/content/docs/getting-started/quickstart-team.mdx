...
description: Set up pair for a development team with shared standards, adoption files, and multi-agent support.
---
...
- A shared Git repository for your project
- Team members using AI coding assistants (Claude, Copilot, Cursor, or similar)

## Install pair-cli

Each team member installs pair-cli globally:
...
This creates the `.pair/` folder with the Knowledge Base and the agent bridge pattern files. Commit the result to your shared repository so every team member gets the same context.
...
- Explore the full [process lifecycle](/docs/concepts/process-lifecycle) your team can follow
