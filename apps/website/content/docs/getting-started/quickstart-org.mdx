...
In **this central KB repository — and only here** — `.pair/knowledge/` is source you author. Add or modify files in `.pair/knowledge/guidelines/` to reflect your organization's standards:
...
In a **consuming project**, the same path is installed output: the `knowledge` registry is a mirror, so `pair-cli update` overwrites it and edits there are lost. Teams customize through `.pair/adoption/` instead — see [Customize for Your Team](/docs/customization/team) and [Customize Templates](/docs/customization/templates). The distinction matters for every rollout: your org's standards belong in this repo's KB and reach teams through the package, never through hand-edits on their side.
...
- **[Enterprise Adoption tutorial](/docs/tutorials/enterprise-adoption)** (~60 min) — guided rollout walkthrough for organizations
- **Own the knowledge base as a repository**: [External KB Repository](/docs/customization/external-kb) — scaffold a versioned, releasable KB your projects install from, instead of copying guidelines per project
