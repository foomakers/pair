...
Every other option on this page starts with an account. Sign up, create a workspace, provision an API key, wire an MCP server — and only then can pair file your first story. That is a reasonable price for a team. It is a poor one for a developer trying pair out on a Saturday, for anyone working offline, and for an air-gapped environment where "create an account" is not a step that exists.
