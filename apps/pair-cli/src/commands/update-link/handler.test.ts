...
describe('handleUpdateLinkCommand - working area hard exclusion (D14)', () => {
  const cwd = '/project'

  test('refuses to process a --target that overlaps the working area', async () => {
    const fs = new InMemoryFileSystemService(
      {
        [`${cwd}/package.json`]: JSON.stringify({ name: 'test', version: '0.1.0' }),
        [`${cwd}/.pair/working/checkpoint.md`]: '[link](/absolute/path)',
      },
      cwd,
      cwd,
    )
  ...
  test('refuses to process when a configured working_path override lands inside --target', async () => {
    const fs = new InMemoryFileSystemService(
      {
        [`${cwd}/package.json`]: JSON.stringify({ name: 'test', version: '0.1.0' }),
        [`${cwd}/config.json`]: JSON.stringify({ working_path: '.pair/knowledge/working' }),
        [`${cwd}/.pair/knowledge/file1.md`]: '[link](/absolute/path)',
      },
      cwd,
      cwd,
