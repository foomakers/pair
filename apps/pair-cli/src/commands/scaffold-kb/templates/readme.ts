...
function layoutSection(): string[] {
  return [
    ...
    'scripts/release.sh # packages + publishes a version (wraps `pair-cli package`)',
    '```',
    '',
...
function releaseSection(identity: KbIdentity, host: KbHost): string[] {
  const common = [
    ...
    '```',
    '',
    'The script runs `pair-cli package` (the same command the official KB uses) and',
...
function consumeSection(identity: KbIdentity): string[] {
  return [
    ...
    'from HERE by `pair-cli install --source` AND `pair-cli update --source`: consumers copy',
    'nothing. A consuming project that declares the same registry in its own',
    '`pair.config.json` overrides this one — the source declares, the consumer overrides.',
    '',
    'Registries this KB does not ship (`adoption`, `github`, …) are reported as skipped,',
    'not failed, and the install exit code follows the real failures only. `update`',
    'follows the same tally.',
    '',
