...
function header(identity: KbIdentity, cliVersion?: string): string[] {
  return [
    ...
    '#',
    '# Packaging is delegated entirely to `pair-cli package`: this script only maps a',
    '# version to a ZIP and (on GitHub) to a tag + release. No separate release',
    '# mechanism, so the KB ZIP is byte-for-byte what `pair-cli install` already understands.',
    '#',
