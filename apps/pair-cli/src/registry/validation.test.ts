...
describe('registry validation - working area overlap (D14)', () => {
  ...
  it('fails when a registry target overlaps with the working path', () => {
    const registries: Record<string, RegistryConfig> = {
      pairroot: {
        source: '.pair',
        behavior: 'mirror',
    ...
    const result = validateAllRegistries(registries)
    expect(result.valid).toBe(false)
    expect(result.errors.some(e => e.includes('working area'))).toBe(true)
  })
