...
**Important:** For all operational aspects of initiative creation, update, and management, refer to the Collaboration and Process Guidelines ([Collaboration and Process Guidelines](../tech/knowledge-base/12-collaboration-and-process-guidelines.md)). This document provides the authoritative process for managing initiatives, including the required template, naming conventions, and workflow standards.

If the project management tool is not defined, the AI assistant must always ask the developer which system they prefer (file-based or external tool) before proceeding with any initiative creation. Do not assume a default preference.
Complete the [bootstrap checklist component](./02-how-to-complete-bootstrap-checklist.md) if not done before starting initiative creation.
The AI assistant must always document the project management tool choice in the adoption file (e.g., `.pair/tech/adopted/way-of-working.md`) as soon as the decision is made, to ensure traceability and compliance with the process.
