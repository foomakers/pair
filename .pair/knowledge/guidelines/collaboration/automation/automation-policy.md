...
Against the `## Eligibility` section body **and** the value extracted from it, a consumer **MUST HALT** when any of these holds (triggers 1, 2 and 7 are properties of the body; 3-6 of the extracted value):
...
7. the file carries **more than one `## Eligibility` heading** — not exactly one declaration, so there is nothing to extract without inventing a precedence rule the next consumer will not share.
