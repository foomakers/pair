...
- **No coverage regression**: the in-place update is **not unconditional**. Before overwriting, read the panel already at that path: if it records **higher coverage** for the same period and the same input revision (a complete panel carries no `Coverage` line at all), **keep it** — merge in the rows this run adds where they are separable, otherwise leave the file untouched — and report the shortfall in the **run output** instead of the file. Without this, a context-tight partial re-run silently destroys a complete panel's figures (a `40 of 73` run overwriting a `73 of 73` one), a reader comparing periods reads the resulting drop as real movement rather than a run artifact, and "same inputs ⇒ same content" would depend on how far a run happened to get.
