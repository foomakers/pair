...
| `concludeCheck`   | code-host      | `concludeCheck({ pr, sha, repo, state, description, targetUrl })` — `state` ∈ `success \| failure \| pending`, posted on the given `sha`; `context` defaults to the adapter's own `checkContext` but is honoured if the caller passes one — cycle callers never do. A well-behaved new adapter refuses to publish when `sha` is not the current PR head, REPORTING `{ published: false }` under its own `checkContext` instead of throwing (the worked example below does this); `github.mjs` posts on whatever `sha` and `context` it is given; `azure-devops.mjs` maps `sha` to its PR iteration and reports `published: false` when the sha is not one of them, but does not require it to be the current head — no cycle caller ever supplies a foreign one. Returns `{ context, sha, state, published, error }` |
...
- **Fail typed.** Throw `HostError` (`failed`, `invalid-json`, `invalid-output`, `unsupported`). `merge`'s `MERGE_STRATEGIES` (exported by `adapter-kit.mjs`) is the only accepted set of `strategy` values, and both shipped adapters enforce it — an unsupported `strategy` throws `unsupported`. `concludeCheck`'s `CHECK_STATES` (same export) is what a well-behaved new adapter refuses a `state` outside of, as the worked example below does; the shipped adapters pass `state` through unchecked — `pr-state.mjs conclude`, the only cycle caller, passes only `success | failure`. Any other exception is reported as an adapter bug, with the method name.
