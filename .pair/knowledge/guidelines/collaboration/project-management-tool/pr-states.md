...
| **Review** (`/pair-process-review`) | judgment on the change | a verdict (APPROVED / CHANGES-REQUESTED) in the native code-host review | **zero mechanical checks** — it reads gate results, it never re-runs them |
...
The synthesis is a table, not a heuristic. It is implemented once, in the shipped, provider-agnostic [`pr-state.sh`](../../../assets/pr-state.sh) (`resolve_pr_state`), so every consumer — `/pair-process-review`, `/pair-capability-publish-pr`, a code-host job — computes the same state from the same inputs.
...
| **Self-authored PR** (solo maintainer) | The code host rejects APPROVE on your own PR, so the verdict is recorded as a review comment led by the verdict token (see `/pair-process-review`'s degradation). The `pair-review` check still carries the machine-readable conclusion, so enforcement is unaffected — 🟢 and 🟡 work normally. **At 🔴 the primary path is unavailable:** it asks for a non-author human approving review, and the author cannot supply one, so with that path alone a single-maintainer repository can reach no 🔴 `ready-to-merge` — it needs a **second human account**. The **approval token** ([#398](https://github.com/foomakers/pair/issues/398)) is the alternative satisfaction path for exactly that repository shape: the maintainer posts `/approve <head-sha>` on the PR and the job accepts it after resolving the actor **server-side** — a human account, not an app, whose **repository permission** is read back from the collaborators API as `admin`/`maintain`/`write` (the `author_association` is only a pre-filter: `MEMBER` means "in the owning org", not "has push access here") — and binding it to the current head. On a repository that has **not** declared itself single-human (`PAIR_SOLO_APPROVAL_TOKEN`, compared case-insensitively), the token additionally excludes the PR author, exactly as the review path does — and when the author's token is what that exclusion dropped, the published status says so and names the variable, instead of reporting "no token was posted" to the person who just posted one. See the [solo-maintainer token](github-implementation.md#the-solo-maintainer-approval-token-single-account-repositories) for the host mechanics. |
| **What the solo token means** | **Explicit human confirmation, not independent review.** On a single-account repository there is no second pair of eyes and the flow does not pretend there is: the token buys **deliberateness** (a distinct explicit act), an **audit trail** (who, when, on which head SHA) and **invalidation on change** (a force-push voids it, exactly like an approving review). It is **not forgery-resistant** while the agent runs on the maintainer's own credentials — host-side they are one actor — which becomes achievable only with a dedicated agent identity ([#218](https://github.com/foomakers/pair/issues/218)); ADR-018 § Amendment states this in those words. Where a second human exists, the **review path stays primary and preferred**: it is queried first and wins wherever an approval exists. It is not, however, unreachable there — before any review is submitted (the state of every 🔴 PR at open) a **non-author** write-level human's token also satisfies 🔴, with no approving review recorded in the Reviews tab. What the default rules out is the **self**-approval shortcut, not the token itself; ADR-018 § Decision records why the looser default was kept. And the tier **requirement** is untouched — quality-model §4 and D10 still demand explicit human approval at 🔴; this only adds a way to satisfy it. |
