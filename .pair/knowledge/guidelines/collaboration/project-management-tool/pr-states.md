...
- [`review-identity.sh`](../../../assets/review-identity.sh) — the provider-agnostic identity adapter, six entry points: `review_identity_kind_ok` (the adoption value parsed, so a present-but-unparseable key HALTs instead of degrading to `none`), `resolve_identity_mode`, `review_identity_exclusion_ok` (the checked 🔴 precondition — omit it from a host adapter and a bot-user identity with no `REVIEW_IDENTITY_LOGIN` resolves to `identity` and can sign the 🔴 approval), `identity_verdict_event`, `pair_review_publication_mode`, `identity_audit_comment`
