...
A composing step reaches this step because ITS algorithm composes it — `/pair-process-refine-story` composing `/pair-capability-map-subdomains`, `/pair-process-plan-tasks` composing `/pair-capability-map-contexts`. The user did not name it, so **there is no question to ask them**: a composed step that stopped to ask would make `/pair-process-refine-story` under `poc` interrupt itself over a step the user never mentioned.
...
- **A required composed step is not silently skipped.** The not-installed rule already distinguishes these: an **optional** composed step degrades, a **required** one HALTs. A profile that disables a step another enabled step requires is the prerequisite inconsistency `/pair-next` reports with its minimal fix — a configuration error to fix in `way-of-working.md`, not something to resolve mid-run.

## What stays in the skill (the delta)

A `## Process Profile` section at the skill's invocation entry point, carrying its **declared step id** — the marker is the contract `skills:conformance` checks, and the sentence is what the executor reads. This is the block the corpus ships, quoted from `/pair-process-refine-story` verbatim (a conformance test pins the two together, so this snippet cannot drift from the twelve deltas it governs):
