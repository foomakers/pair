...
Used by the 8 **decision** skills — `assess-ai`, `assess-architecture`, `assess-infrastructure`, `assess-methodology`, `assess-observability`, `assess-pm`, `assess-stack`, `assess-testing`. Each proposes a choice to be persisted via [`/pair-capability-record-decision`](record-decision-contract.md).
...
- Proposes a choice for `/pair-capability-record-decision` to persist → **Decision Shape**.
