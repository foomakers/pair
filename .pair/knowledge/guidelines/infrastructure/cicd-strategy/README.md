...
### 🎚️ Tier-Aware Pre-Merge Pipeline

**[Tier-Aware Pre-Merge Pipeline](tier-aware-pipeline.md)** - Opt-in risk-tier-modulated gate that reads classification tags only (default: the full check suite runs on every PR)

**Opt-in optimization**, enabled via the `Pre-merge tiering` flag (default `disabled` — the full suite runs on every PR). When enabled: the pre-merge gate matrix (base every tier, unit from 🟡, integration/E2E from 🔴) is driven purely by the PR's `risk:*` tag — no classification criteria in CI (D18), fail-safe 🔴 when untagged, explicit failure on a missing suite, and a build+deploy-only post-merge staging pipeline. Generated by `/pair-capability-setup-gates`; criteria sourced from the quality model.
