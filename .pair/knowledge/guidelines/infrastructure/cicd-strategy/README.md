...
**[Tier-Aware Pre-Merge Pipeline](tier-aware-pipeline.md)** - Risk-tier-modulated gate that reads classification tags only

The pre-merge gate matrix (base every tier, unit from 🟡, integration/E2E from 🔴) driven purely by the PR's `risk:*` tag — no classification criteria in CI (D18), fail-safe 🔴 when untagged, explicit failure on a missing suite, and a build+deploy-only post-merge staging pipeline. Generated by `/pair-capability-setup-gates`; criteria sourced from the quality model.
