...
**Skill**: `/pair-capability-map-contexts` is a **capability**, not a standalone process step — it is invoked scoped (`$scope` = the contexts/services just touched) by `/pair-process-refine-story`, `/pair-process-plan-tasks`, or `/pair-process-bootstrap` (full scope). When available, it automates the operational steps of this workflow (constraint extraction, context catalog, relationship assessment, file creation). This how-to describes the workflow and its HALT conditions.

## Skill Composition

This how-to is realized by the `/pair-capability-map-contexts` capability, invoked by another skill.

| Skill            | Purpose                                                                                                   |
| ---------------- | --------------------------------------------------------------------------------------------------------- |
| `/pair-capability-map-contexts`  | Executes the scoped process: subdomain synthesis, context boundary analysis, relationship assessment, BC file creation. |
...
2. **Invoke `/pair-capability-map-contexts`** with `$scope` set to the contexts/services just touched (`all` only when composed by `/pair-process-bootstrap`). The skill handles:
