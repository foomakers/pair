...
| Skill           | Purpose                                                                                                                                                         |
| --------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `/pair-process-refine-story` | Executes the full refinement process: phase 0 grill sync, requirements (Given-When-Then), technical analysis, classification, sprint readiness, documentation. Section-level idempotent. |
| `/pair-capability-grill`        | Composed by `/pair-process-refine-story` — phase 0 blocking AI↔human shared-understanding sync before any DoR section is authored (R3.11 alignment gate).                     |
| `/pair-capability-map-subdomains` + `/pair-capability-map-contexts` | Composed by `/pair-process-refine-story` — scoped domain/context placement for the story; feeds the classification matrix and the coupling dimension. |
| `/pair-capability-classify`     | Composed by `/pair-process-refine-story` — writes the shift-left classification (risk/cost) matrix into the story's `## Classification` section.                              |
...
2. **Invoke `/pair-process-refine-story`** with optional `$story` argument. The skill handles:
   ...
   - **Phase 0 shared-understanding sync** via `/pair-capability-grill` — a blocking R3.11 alignment gate; no DoR section is authored until AI and human agree (HALT otherwise)
   ...
   - Technical analysis (architecture alignment, risks, spike identification) with scoped `/pair-capability-map-subdomains` + `/pair-capability-map-contexts` placement
   - Classification matrix via `/pair-capability-classify` written into the story's `## Classification` section
