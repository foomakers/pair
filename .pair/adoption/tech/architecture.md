...
- **The routing core is host-agnostic and credential-free.** It lives in the `pair-cli run` entry point (ADR-021 tier 2) as a pure function over the card, the labels a trigger observed, and the policy; the labels are an INPUT (`--card`/`--card-tags`) supplied by a thin per-host trigger adapter, never fetched by the driver. Adding a code host is a new adapter, never a change to the core.
...
- **The cycle's host writes go through one PM/code-host adapter layer.** `scripts/host/` (shipped in every workflow skill) holds one adapter per tool — GitHub and Azure DevOps today — behind eight methods; `cycle-state.mjs`, `pr-comment.mjs` and `pr-state.mjs` never spawn a host CLI. The coordinator binds `pm-tool` / `code-host` once per run (ADR-018 resolution, `bind-hosts`), a declared host without an adapter HALTs `host-unsupported`, and a new host is a new file. See [ADR-026](adr/adr-026-pm-code-host-adapter-interface.md).
