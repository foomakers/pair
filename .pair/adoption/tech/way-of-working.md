...
- **Link gates resolve targets case-sensitively, on every OS.** `pnpm --filter @pair/knowledge-hub check:links` (KB-internal relative links) and the docs-site `docs:staleness` Check 5b (repo-file citations) compare each path segment byte-for-byte against its parent directory instead of trusting `fs.stat`, which is case-insensitive on APFS: a miscased link is a 404 on github.com and an ENOENT on Linux CI, so a local PASS must mean the same thing. See ADL [2026-09-03-kb-link-gate-resolves-targets-case-sensitively.md](../decision-log/2026-09-03-kb-link-gate-resolves-targets-case-sensitively.md). **Check 5b resolves a repo citation ref-deep and anchor-deep**, not just path-deep: the ref is read from the URL (`main` resolves; an immutable sha/tag permalink is skipped; any other ref — `master`, `mian` — fails), and a `#fragment` on a `blob/main/*.md(x)` target must match a heading slug computed the way github.com computes it. See ADL [2026-09-03-repo-citations-are-checked-ref-and-anchor-deep.md](../decision-log/2026-09-03-repo-citations-are-checked-ref-and-anchor-deep.md). **Both link checks read the RENDERED surface, not the raw `.mdx` bytes** — a repo URL inside a fence, a code span or a `{/* … */}` JSX comment (either of the latter two spanning MULTIPLE lines) is literal text and is not gated, so a page can quote the counter-example it is teaching and can comment a link out the canonical MDX way — and the oracle for that surface is **the site's own build** (fumadocs/MDX), not github.com: the two disagree on four measured rows, so the shared reader is used in its `mdx` flavour (no § 4.6 HTML blocks, no § 4.4 indented code). **The scope a construct is masked in comes from the reader, never re-derived in the gate**: a leaf event answers `blockStart` (many shapes interrupt a paragraph, not just an ATX heading — under `mdx` any JSX flow element does, a GFM table row does on both renderers, and a 4-space-indented line or a `2.` marker does on neither) and a table row hands over its `cells`, because each cell is a separate inline scope; a `{/* … */}` comment that STARTS a line is the reader's `expression` block under `mdx`, so a fence marker inside it never opens a fence (the gate's leaf-text mask handles only a comment that starts mid-line). See ADL [2026-09-04-link-surface-oracle-is-the-site-build.md](../decision-log/2026-09-04-link-surface-oracle-is-the-site-build.md). **The link surface, the citation resolver, the github.com slugger and their lossless diagnostics live in `apps/website/lib/repo-citations.ts`**, re-exported by `docs-staleness-check.ts` (which keeps the orchestration: skill counts, catalog, CLI commands, list-targets samples). See ADL [2026-09-07-repo-citations-is-its-own-module.md](../decision-log/2026-09-07-repo-citations-is-its-own-module.md).
