...
- **`format` required check**: `format` is the third context in the "declared but not yet required" set, alongside the two above — CI runs `pnpm format:check` on every pull request and on push to `main` via its own [`format.yml`](../../../.github/workflows/format.yml) workflow (#413), reporting as the status context `format`. A **dedicated workflow, not a job in `ci.yml`**: that workflow's `paths-ignore: ['.changeset/**']` is declared at workflow level and a job inherits it, so a `.changeset/**`-only PR would run no formatting check — trigger coverage is part of check coverage, and `format.yml` therefore declares no `paths-ignore` (guarded by `format-workflow-composition` in `@pair/dev-tools`, which also bans `pull_request_target`, secrets, `${{ }}` in a `run:`, and any write-mode step: CI reports, it never repairs). Status on this repo: **not yet applied** as a required check — same pending admin-scope step, and the same ordering constraint, as the row above; until then it reports and is advisory. Unlike `pair-explicit-approval` it needs **no `app_id` pinning and no second human account**: it asserts a mechanical property of the tree, so it is an anti-accident control like `pair-review`, not an authorization control.
