...
- **Coverage guardrail:** test coverage is protected by a baseline regression gate consumed by the pre-merge pipeline — a drop below the established baseline blocks merge, maintaining/improving passes (not an absolute wall). Per-type targets + baseline live in [coverage-baseline.md](coverage-baseline.md) (KB defaults documented in [coverage-config-example.md](../../knowledge/assets/coverage-config-example.md)); the shipped [coverage-gate.sh](../../knowledge/assets/coverage-gate.sh) reads them. Wiring + fail-safe (block at red / warn lower on a missing report) and baseline bootstrapping: [tier-aware-pipeline.md](../../knowledge/guidelines/infrastructure/cicd-strategy/tier-aware-pipeline.md#coverage-guardrail-regression-gate-consumed-by-this-pipeline).
