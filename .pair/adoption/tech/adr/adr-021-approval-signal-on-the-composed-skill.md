...
- **Pros**: the caller states its depth **once** and cannot miss a surface, because the obligation moved to the composed skill. The default makes the change **guided-neutral by construction** (an absent argument resolves to the text that was already there). Not *provably*, and the distinction is worth keeping: the shape guarantees it, the enforcement is a phrase heuristic (Trade-offs below), and the one time it was broken — a directive placed on the wrong side of the `auto` clause — the shape still held while the prose did not. The three-kind resolution derives the surviving HALT from the mechanism instead of carving it out by name — a gate is where the skill *cannot state what the human would be accepting*, so there is nothing to auto-accept. And it is **mechanically enforceable**: an argument row plus a signal named in the asking step are both greppable over the corpus, per skill present, with no count anywhere.
- **Cons**: it is a **new argument on eleven skills** (nine `assess-*` members with a round, plus both `map-*`), and the enforcement rests on a **textual** convention (an approval-round line must name the signal) — a novel prompt phrasing the detector's pattern set does not recognise is an approval round the gate will not see. That is not hypothetical: the first implementation pass qualified the *confirmation* rounds and missed three *tie-break* rounds ("present top 2 with trade-off analysis", "ask developer to choose") which the pattern set also did not match, so the gate was green over a hang until review found it. Mitigated by keeping the pattern set in one tested production module with injection tests — extended to choice rounds, and to a family skill's sub-docs, for exactly that reason — and by the shared rounds being qualified in the cascade rather than restated eleven times. Second cost: a caller must now *pass* something it previously only documented, so a caller that neither passes nor discloses is silently interactive again — visible in a hang, not in a red gate.
...
The mechanism **belongs to the convention, not to a caller**. The convention is stated once in `approval-rounds.md`; the cascade doc qualifies the two rounds it owns; each skill declares the argument and qualifies only its own local rounds. `/pair-process-bootstrap`'s quick depth passes `$approval: auto` and its disclosed deviations 2 and 3 are retired — the caller-side notes in Steps 2.2 and 3.5 with them, since they existed only to describe those deviations.
...
- The surviving `/pair-capability-map-contexts` HALT is derived, not excepted: a round with no proposal to accept is a gate, and `auto` suppresses asking, never judging.
...
- **Eleven skills** — the nine `assess-*` members with an approval round (`assess-security` included, whose only rounds are its audit-mode Path A/B) plus both `map-*` skills: an `$approval` argument row and each local round qualified, confirmation and tie-break alike.
- **`/pair-process-bootstrap`** — quick mode composes `assess-*` and `map-*` with `$approval: auto`; `quick-mode-defaults.md` deviations 2 and 3 and the Step 2.2 / 3.5 caller-side notes are retired.
