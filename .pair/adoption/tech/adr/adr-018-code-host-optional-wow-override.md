...
- `/pair-capability-write-issue` gains `$mode: comment` — the only sanctioned back-link mechanism through that skill — which is non-destructive and warns rather than HALTs; where it is not installed, `/pair-capability-publish-pr` writes the comment directly through the PM tool's implementation guide.
- **Back-link dedup is the caller's, not the writer's**: a comment has no identifier, so `$mode: comment` cannot be idempotent; `/pair-capability-publish-pr` therefore checks the item for a comment already carrying the PR URL and skips posting when it finds one. Without this the fix→re-publish loop would accrete one back-link per round.
- **Identifier equality is per product, not per spelling**: the resolution convention carries the canonical alias list (`github` ≡ `github-projects`, `azure-devops` ≡ `azure-repos`/`azure-boards`, `gitlab` ≡ `gitlab-issues`), so a project naming the same product in both fields always takes the single-tool path — never a prose assertion.
...
- **Negative / accepted**: an existing `linear`/`jira`/`filesystem` adoption receiving this change through `pair update` must declare `code-host` **once** before its next PR operation (or re-run `/pair-capability-setup-pm`, which backfills it) — the single one-time upgrade step this decision imposes, flagged in the schema, the resolution convention and the concept page; eight skills now carry a routing pointer they didn't need before; a hand-edited PR body can break the `Refs:` read-back; the split path ships verified by documentation + content assertions, **not** by a live Linear+GitHub cycle (no throwaway Linear workspace was available) — recorded as an accepted gap on story #236 and disclosed in PR #389 rather than silently left unchecked.
