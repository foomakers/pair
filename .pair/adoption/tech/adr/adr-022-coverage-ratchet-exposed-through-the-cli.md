...
- [tier-aware-pipeline.md](../../knowledge/guidelines/infrastructure/cicd-strategy/tier-aware-pipeline.md) (+ dataset twin): the pair-internal caveat is **removed** (it describes a limitation this decision ends), the commit-back step is added to the pipeline template, and the Actions-specific parts are named.
...
- [setup-gates](../../../../.claude/skills/pair-capability-setup-gates/SKILL.md) (+ dataset twin): asks the nested question only when the parent guardrail is enabled, records the flag, and emits the step only when it is on.
