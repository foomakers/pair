...
- **Description**: `/pair-capability-setup-gates` asks the nested question and emits a step that invokes a project-declared command (`PAIR_RATCHET_CMD`), documenting the contract pair's own step satisfies.
...
- **Description**: The implementation stays one unit-tested TypeScript module and moves to `apps/pair-cli/src/commands/coverage-ratchet/ratchet.ts`, behind a new `pair coverage-ratchet` command (metadata + parser + thin handler). `/pair-capability-setup-gates` emits a step that invokes it with a pinned `npx --yes @foomakers/pair-cli@<version>`; pair's own CI step invokes the same command from the dist it built earlier in the job.
