...
- ~~**A distinct reviewer identity stays out of scope**, deliberately (Option 4): the dispatched reviewer
  ...
  **adds** that login clause — the one shipped-predicate change it makes, additive and inert while the
  ...
  The solo-approval token is no longer a separate design question: the **2026-08-30 amendment below**
  ([#398](https://github.com/foomakers/pair/issues/398)) adopts it as a second *satisfaction path* for
  the same human requirement — explicit human confirmation, not independent review — and it needs this
  story's identity only for forgery-resistance, not as a prerequisite.
