...
7. **A dispatched card IS the run's scope — under the routed workflow's own name for it.** The card travels as an argument the workflow declares (`--root` for `pair-loop`, `--story` for `pair-process-refine-story` and `pair-process-plan-tasks`), borrowed from its `## Arguments` table and never invented (D18); the mapping from the driver's scope slot to each workflow's spelling is DATA in `invocation.ts`, pinned against the dataset corpus by a test. There is no safe default for a workflow the driver has no row for: an argument a skill does not declare is *ignored*, not rejected, and both `pair-process-*` workflows then select the highest-priority story on the board themselves — so the run would work a card nobody tagged while the audit trail and the `DISPATCH-RECORD:` comment both named the card that was tagged. Refusing the mapping is the only outcome that keeps item 4's record true.
...
- **A mapping naming an uninstalled workflow — or one the driver cannot scope to a card — HALTs the whole board**, not just the cards carrying that tag: both checks run before eligibility and routing. The second bounds what a mapping may name to the workflows the driver holds a scoping row for (the KB catalog's three, today): a project mapping a tag to any other skill is refused rather than dispatched blind, and widening that set is a one-line data edit plus the corpus assertion that keeps it honest. Deliberate — a broken mapping is broken configuration, and surfacing it only on whichever card happens to carry the tag would make the failure depend on which trigger fired first — but the blast radius is a property adopters must be told about, so it is stated in both the schema and the adapter's pre-flight.
