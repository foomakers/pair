...
- A bounded flatten cannot produce the traversal-unsafe tail an unbounded one made impossible: `.`/`..` in the preserved tail is rejected rather than joined onto the destination root.
