...
2. **The loop is a workflow, not in-context iteration. Two workflows, one per story:**
   ...
   - **#250 — `pair-loop`**: the loop engine. Per iteration it selects eligible cards via `pair-next`, runs a **dependency analysis**, and **composes `implement-batch`** to drive a mutex-safe batch in parallel when warranted (else sequential); enacts the automation policy; evaluates the stop predicate. The dependency analysis is transparent (surfaced in the audit) and overridable (the user can pin cards sequential or exclude them); parallelism is bounded by a user-set `max_parallelism` cap (see §6). `pair-loop` **absorbs the supervised behaviour** (eligibility + audit) — #219 is no longer a separate "supervisor".
