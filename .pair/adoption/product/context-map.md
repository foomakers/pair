...
- **Open — DoD vs DoS naming.** The canonical-terms annotation abbreviates the Done gate as "DoS" while the gap analysis and every template use "DoD" (Definition of Done). Both denote the same concept; registered here as an open naming question until a decision settles it (edge case: the map tracks reality, competing labels side by side). Ref: triage line 108 vs G1.
