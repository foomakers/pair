...
- `publish()`'s derivation is `rank(finding.severity) >= rank(policy.blockingFloor)`, ranked by the review draft's own top-level `severityRanks` (the template contract's exact names) when the draft supplies one, else pair's default table (`Critical`/`Blocker` 4 > `Major` 3 > `Minor` 2 > `Questions` 1, case-insensitive). A severity no rank covers blocks (fail-safe); a floor no rank covers releases nothing (fail-safe — every finding stays blocking). The `Blocker` name is not a special alias outside this table — it is simply one of the default table's own entries, ranked equal to `Critical`.
