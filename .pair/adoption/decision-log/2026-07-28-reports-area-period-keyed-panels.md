...
- **Partial coverage, and no coverage regression**: a period too large for one run still gets its panel, with the shortfall (`Coverage: N of M ... processed` + reason + the items left out) in the headline — but a run whose coverage is *lower* than the panel already on disk for the same period and input revision must not overwrite it: keep the more complete panel (merging separable new rows) and report the shortfall in the run output. Figures count the processed items; the found count appears only in the `Coverage` line.
