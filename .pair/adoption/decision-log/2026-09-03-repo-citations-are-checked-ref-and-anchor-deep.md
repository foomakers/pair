...
- A dead-anchor diagnostic must be **losslessly readable AND actionable**: every code point outside printable ASCII is rendered `\u{XXXX}` (a literal backslash doubled), and the nearest headings are offered as `did you mean #…?`. Raw, `CLAUDE.md#-essential-commands` (dead) and `CLAUDE.md#️-essential-commands` (live) print IDENTICALLY, so a red build reads as a false positive and the developer deletes a working fragment.
- **A candidate is offered in BOTH spellings** — the escape plus a raw `(copy: #…)` form, omitted only when escaping is a no-op. Escape-only advice was a dead end that could not be followed: typing the ASCII `#\u{FE0F}-essential-commands` leaves the citation dead and, being 8 code points from the real slug (`editDistance`, budget 3), strips the candidate from the second message too. Proven by applying the message's own bytes: red → paste the `copy:` form → `docs:staleness` exits 0, and the pasted anchor is byte-for-byte the `href="#…"` github.com serves for that heading.
- **Proximity is relative, not absolute**: a candidate must be within Levenshtein 3 over code points AND within half the longer of fragment/slug (at most 3, nearest-then-lexicographic). An absolute-3 bound made every 3-character slug "near" any 3-character fragment — `#zzz` in a file of `## Cat`/`## Dog`/`## Elk` came back `did you mean #cat or #dog or #elk?`. Following that advice yields an anchor that RESOLVES, so the gate prints PASS while the reader lands on an unrelated section: the wrong-destination anchor this check exists to catch, induced by the check.
