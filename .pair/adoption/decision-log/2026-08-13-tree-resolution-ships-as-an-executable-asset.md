...
6. **Verification follows the gate-tooling ADL (2026-07-13)**: the asset's BEHAVIOR is executed by `scripts/smoke-tests/scenarios/pr-tree-resolve.sh` (`OFFLINE_SAFE`, real git repositories + a stubbed host read), registered in `run-all.sh`'s `--ci` scenario list (the `CI_TESTS` array); `verify-quality.test.ts` keeps only CONTENT invariants, now read from the file the step sources. **Execution and registration are two different mechanisms, and neither is an automated trigger yet.** What executes the scenario today: `pnpm smoke-tests` is `./scripts/smoke-tests/run-all.sh --cleanup`, which leaves `IS_CI=false` and therefore **does not consult `CI_TESTS` at all** — it globs `scenarios/*.sh` and runs every scenario present. That suite is a **required custom gate** in this repo's [Custom Gate Registry](../tech/way-of-working.md) (order 2), so an agent running `/pair-capability-verify-quality` executes the scenario *because the file exists in `scenarios/`*, not because it is listed. What the registration buys: membership in the CI-safe subset that `run-all.sh --ci` (optionally `--offline-only`) runs — the entry point [#400](https://github.com/foomakers/pair/issues/400) will wire, and the one whose scenario set is curated rather than globbed. Anyone wiring CI per this record should invoke `run-all.sh --ci`, not `pnpm smoke-tests`. Meanwhile **no workflow invokes `run-all.sh`** in any mode, and `pnpm quality-gate` (the pre-push hook's whole content) does not include `pnpm smoke-tests`. So the arms are executed by a gate/local run, never by CI and never by the hook: nothing mechanical stops a merge on a break in them. Until #400 lands, this asset's regression protection is the custom-gate/local run plus the content invariants — not a merge gate, and the record says so rather than letting "registered" be read as "enforced" (conformance-pinned in both directions: the claim may strengthen only when a workflow, or the gate command itself, actually runs the suite).
...
- Dataset/KB: new `assets/pr-tree-resolve.sh` in both knowledge trees; `verify-quality/SKILL.md` sources it; `scripts/smoke-tests/` gains `pr-tree-resolve.sh` and its registration in `run-all.sh`'s `--ci` scenario list (`CI_TESTS`). That list is read only by `run-all.sh --ci`; the gate/local `pnpm smoke-tests` run does not consult it and executes the scenario by globbing `scenarios/` — so registration buys membership in the CI-safe subset #400 will invoke, not today's execution (see decision 6).
