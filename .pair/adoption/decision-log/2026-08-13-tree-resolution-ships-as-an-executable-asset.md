...
6. **Verification follows the gate-tooling ADL (2026-07-13)**: the asset's BEHAVIOR is executed by `scripts/smoke-tests/scenarios/pr-tree-resolve.sh` (registered in the CI list, `OFFLINE_SAFE`, real git repositories + a stubbed host read); `verify-quality.test.ts` keeps only CONTENT invariants, now read from the file the step sources.
...
- Dataset/KB: new `assets/pr-tree-resolve.sh` in both knowledge trees; `verify-quality/SKILL.md` sources it; `scripts/smoke-tests/` gains `pr-tree-resolve.sh` and its CI-list registration.
