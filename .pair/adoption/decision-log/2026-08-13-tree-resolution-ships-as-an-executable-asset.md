...
6. **Verification follows the gate-tooling ADL (2026-07-13)**: the asset's BEHAVIOR is executed by `scripts/smoke-tests/scenarios/pr-tree-resolve.sh` (registered in `run-all.sh`'s `pnpm smoke-tests` list, `OFFLINE_SAFE`, real git repositories + a stubbed host read); `verify-quality.test.ts` keeps only CONTENT invariants, now read from the file the step sources. **Registration is not yet an automated trigger.** What that list buys, exactly: `pnpm smoke-tests` is a **required custom gate** in this repo's [Custom Gate Registry](../tech/way-of-working.md) (order 2), so an agent running `/pair-capability-verify-quality` executes the scenario — but **no workflow invokes `run-all.sh`**, and `pnpm quality-gate` (the pre-push hook's whole content) does not include `pnpm smoke-tests`. So the arms are executed by a gate run, never by CI and never by the hook: nothing mechanical stops a merge on a break in them. [#400](https://github.com/foomakers/pair/issues/400) owns the CI wiring; registration is the precondition it builds on. Until it lands, this asset's regression protection is the custom-gate/local run plus the content invariants — not a merge gate, and the record says so rather than letting "registered" be read as "enforced" (conformance-pinned in both directions: the claim may strengthen only when a workflow, or the gate command itself, actually runs the suite).
...
- Dataset/KB: new `assets/pr-tree-resolve.sh` in both knowledge trees; `verify-quality/SKILL.md` sources it; `scripts/smoke-tests/` gains `pr-tree-resolve.sh` and its registration in `run-all.sh`'s `pnpm smoke-tests` list (a manual entry point today — see decision 6 and #400).
