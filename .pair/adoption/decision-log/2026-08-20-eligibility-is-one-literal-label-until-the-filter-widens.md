...
- The seven checks are **inexact in both directions**, and the guideline now says so on both sides. Under-inclusive: two colon-free labels on one line pass every check. Over-inclusive: triggers 3 and 4 HALT a *legitimate* single label whose name starts with `-`/`*`/`+`/`>`/`#` (`#tech-debt`, from a tag-style Tag Projection) or carries a comma — indistinguishable by shape from decoration or a list. Both residuals are fail-safe (nothing is ever widened); the remedy for the over-inclusive one is to rename or re-project the label, not to loosen the trigger.
- Heading identity is pinned so two conforming consumers cannot disagree on the same file: the match is **level 2 exact** (`### Eligibility` is not the declaration — it leaves the section absent), and headings are counted as **rendered markdown** (a `## Eligibility` inside a fenced code block is display, not a heading), so a line-scanning consumer and a parser-based one count the same.
