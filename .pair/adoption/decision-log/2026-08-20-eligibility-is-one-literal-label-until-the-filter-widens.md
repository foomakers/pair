...
Eligibility is **exactly one literal label**, matched by string equality against the emitted labels. A comma, a second non-empty line, or a standalone `AND` / `OR` / `NOT` token is a broken adoption file and HALTs the consumer — it is never partially honoured. Widening this is a **separate story that widens `pair-next --filter` first**; this schema follows it, never leads it.
...
- `automation-policy.md` defines malformed operationally (empty / more than one non-empty line / comma or operator) and HALTs on it; `automation-eligibility.test.ts` pins the rule and forbids any example filter carrying an operator.
