...
- **The scope is "every registry whose install rewrites content", not "the KB".** `.github/agents/**` (the `github` registry: `behavior: "mirror"`, no flatten/prefix) is the *same relationship* — `applySkillRefsToNonSkillRegistries` runs the identical skill-ref + link-path rewrite over its three `*.agent.md` files — so it is guarded by the same data-driven suite, listed in `REWRITTEN_MIRROR_REGISTRIES`, rather than left to a second guard that would drift from this one. Only two paths per registry are registry-specific. The registries deliberately left out are the ones the comparison would be *wrong* for, and each exclusion is asserted rather than argued: `adoption` is `behavior: "add"` (seeded once, then owned by the adopting project — divergence is the point), `agents` carries a per-target naming transform, and `skills` declares flatten+prefix, which makes the pipeline skip the skill-ref rewrite entirely (that pair is `skill-md-mirror`'s job). A registry's declared `include` narrowing is honored, so an un-copied sibling directory is never demanded of the mirror.
