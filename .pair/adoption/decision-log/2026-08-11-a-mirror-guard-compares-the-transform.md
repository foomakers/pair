...
- Measured across the whole KB corpus on 2026-08-11: **441 markdown files, 440 of them already transformed**. `guidelines/collaboration/templates/code-review-template.md` was the **only** file left byte-identical to its dataset source while the transform would have changed it. The guard was pinning the single anomaly and calling it the rule.
...
The guard's scope is the **class**, not the instance: `kb-mirror.test.ts` asserts `mirror === transform(dataset)` for **every** markdown file the KB dataset contributes (derived from disk at collection time — no list, no count), plus an explicit "no mirror is frozen untransformed" sweep whose failure names the whole offending set. A file the transform legitimately leaves untouched (no skill references) still passes: `transform(dataset) === dataset` there, so the rule does not require every mirror to differ.
...
- The whole `.pair/knowledge/**` tree is now guarded, where previously only `skills-guide.md` was. Adding a KB file adds a case with no test edit.
