...
**`pair update` performs mirror cleanup on the path it actually takes.** `handleMirrorCleanup` is exported from `@pair/content-ops` and invoked from the CLI's registry `copyDirectory` before the copy, for every registry that mirrors **any** path. Four properties are part of the decision, not implementation detail:
