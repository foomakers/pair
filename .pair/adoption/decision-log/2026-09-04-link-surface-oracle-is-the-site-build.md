...
4. **Code spans and `{/* … */}` JSX comments are masked with a SPACE on leaf lines**, so `` [`<url>`](<url>) `` — a code span inside a link — reports the destination exactly once, which is what the site serves.
...
- A docs page may now quote any repo URL inside a fence, a code span or a JSX comment, including `blob/master/…` as a counter-example. The contributing/reference pages can teach the rule.
