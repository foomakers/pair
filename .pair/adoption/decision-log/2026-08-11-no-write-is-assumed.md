...
- A future skill that writes to a tracker inherits the rule by pointing at it — the invariant is prose in one skill plus a conformance guard, not a shared code path. That is the accepted limit of this decision: it binds the corpus, not a runtime. The **existing** direct writer, `/pair-process-implement` Step 0.1b, is not left to inherit it later: decision 11 brings it under the rule now, because it contradicted decision 4 on every story it ran (assign to "the current developer" is exactly the authenticated-user default the cascade forbids).
