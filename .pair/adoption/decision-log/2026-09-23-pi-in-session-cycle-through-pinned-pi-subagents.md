...
- **Deterministic rehydration**: every resumed task opens with the order to read the previous session file named on the host's `Original session file` revive-header line (the pinned `WorkflowScriptChildResult` returns no session file) — the context step no longer depends on the model's choice.
- **Session-bound resume**: `pi-subagents` resolves a retained run only in its parent session, so the bridge's ledger binds each run to `PI_SESSION_ID`; another session's run, an unbound entry, a `not-resumable` child or a host-rejected resume ⇒ same-role fresh stage, said once as `reuse→fresh`.
