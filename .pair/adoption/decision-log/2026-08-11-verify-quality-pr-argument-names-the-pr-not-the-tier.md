...
3. **Tier parity is not result parity.** `$pr` makes the *tier* exact, not the *result*: off-branch the suites still run against the checked-out tree. The report states `Tier source:` and `Tree:` separately, `Tree:` being resolved deterministically (branch name when on a branch, **head commit** when detached), and a `Tree: ⚠️` run is **advisory** for the caller — for the PR state synthesis the authoritative gate signal is CI's check on the PR head commit.
...
- A local gate run whose tree is not the PR's head is advisory, so it can never contribute a green to `ready-to-merge`; the required CI checks on the head commit remain authoritative.
