...
2. **"Trivial" is defined mechanically, checkable from `git diff`**: either every changed file is `.md`/`.mdx` (guarded markdown mirrors included), or every changed hunk in every changed file is comment-only, whitespace-only or formatter-output-only — no changed line altering an executable or declarative statement. Renames, string-literal changes, dependency/version changes, config/data value changes, test-expectation changes and regenerated build artifacts are explicitly **not** trivial, however cosmetic they look: behaviour preservation under a rename is a judgment, not a diff-visible fact.
...
4. **`green` is the only accepted value**; anything else, or a malformed key, warns and is treated as absent (§6's malformed rule, D21) — never a HALT.
5. **No skill change.** The rule lives entirely in `quality-model.md`; `/pair-capability-classify` already applies `## Overrides` qualitatively through the Argument > Adoption > KB-default cascade, so it gains no criterion of its own (D18) — asserted by a conformance test that greps the skill for the triviality vocabulary.
...
- **A subjective definition ("changes nothing important")**: rejected — "how important is this prose" is precisely the judgment the mechanical definition exists to avoid. Accepted consequence: on this repo a *normative* KB rule change is `.md`-only and therefore trivial by this definition. It is deliberately tolerated because the other dimensions still fire — a shared-rule edit reads yellow on Change/diff risk — and `max()` keeps it off green.
