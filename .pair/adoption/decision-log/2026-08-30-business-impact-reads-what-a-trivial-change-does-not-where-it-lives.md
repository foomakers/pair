...
- `packages/knowledge-hub/dataset/.pair/knowledge/assets/risk-matrix-example.md` (and its mirror): the key shown in the example `## Overrides` section, the documented adoption starting point. Because adopters **copy** this text into their own `tech/risk-matrix.md`, its restatement of the definition carries the executable-markdown carve-out verbatim — an unconditional "every changed file is `.md`" here would re-open, in the copied file, the hole point 2 closes in the KB.
- `apps/website/content/docs/reference/quality-gates-configuration.mdx` and `.../quality-model.mdx`: both pages enumerate what `## Overrides` holds, and both listed only the two pre-story families. The key is a **third** family — a dimension-resolution override — so both enumerations name it; the catalog page's own rule is that a new opt-in knob ships with its row.
