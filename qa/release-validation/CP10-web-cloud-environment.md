...
- **Still open**: whether attempts 1 and 2 (different network settings) were also secretly ready underneath, or whether the browser bug is intermittent and sometimes correlates with a real stall, is unknown — neither was cross-checked from a second client before being abandoned. MT-CP1003 (the story end-to-end run) has not yet been attempted from a confirmed-working session.
