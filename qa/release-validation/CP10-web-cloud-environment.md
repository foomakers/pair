...
| ---- | ----------- | ----------------- | -------- | ----- | ------ | ----- |
| 2026-08-20 | Claude Code Web | foomakers/pair (private) | — (not reached) | — (not reached) | `BLOCKED — session provisioning did not complete` | Attempt 1: stalled at the first of four provisioning steps ("Configurazione di un container cloud") for 16+ minutes, network access set to "Attendibili" (trusted). No error surfaced, no progress to "Clona repository". Attempt 2, fresh session, network access widened to full/unrestricted: **same stall, same first step, no further progress.** Because the stall is *before* "Clona repository", nothing repo-specific (`pair`'s setup script, monorepo size, dependencies) can be the cause — the repo is never reached. Network policy is also ruled out by the second attempt. What remains: a Claude Code Web platform-side provisioning fault, or something specific to this account/organization independent of the repo. MT-CP1001 could not start either time: no auth check, no skill check, nothing pair-specific was exercised. Recorded per MT-CP1005 ("no case is silently skipped... a truncated run is reported as PARTIAL, never as a pass"): `BLOCKED`, not `PARTIAL` — no case body ever ran in either attempt.

### Notes

- **This path has been attempted twice and neither completed.** Both 2026-08-20 attempts stalled at the identical provisioning step before the repository was ever cloned — see the row's Notes for what that rules out (repo content, network policy). `pair`'s web support therefore remains _assessed, not verified_: no row above carries a `$STORY`/`$PR` pair, so AC1, AC2 and AC7 of the story that authored this path stay unmet. **This is reported to the maintainer as a genuine defect candidate outside pair's own repo (per MT-CP1006) — likely a Claude Code Web platform issue or an account/org-side block — not something a change to this codebase can fix.** Re-attempt from a different account/org if one is available before assuming the platform itself is at fault.
- **MT-CP1005 gap surfaced by this run**: none of its five degraded-path rows name "session/container provisioning never completes" as a stop condition — they all assume the container is up and something *inside* it fails (`gh` unauthenticated, MCP unavailable, Playwright unavailable, mid-run interruption, a refused operation). A provisioning stall precedes all of those, and two occurrences in a row rule out one-off variance. Add a sixth row for it.
