...
| ---- | ----------- | ----------------- | -------- | ----- | ------ | ----- |
| 2026-08-20 | Claude Code Web | foomakers/pair (private) | — (not reached) | — (not reached) | `BLOCKED — session provisioning did not complete` | Three independent attempts, all stalled at the identical first of four provisioning steps ("Configurazione di un container cloud"), before "Clona repository" ever ran. Attempt 1: network access "Attendibili" (trusted), 16+ minutes, no error. Attempt 2: fresh session, network widened to full/unrestricted — same stall, same step. Attempt 3: fresh session, a different, simple prompt (no commands requested) — same stall, same step. **Three variables were changed one at a time (network policy, prompt content) and none of them changed the outcome.** Because the stall is *before* the repository is cloned and *before* Claude Code itself launches (the sequence is container → clone → setup script → launch Claude Code; the typed prompt is only consumed at the last step), neither the repo's content nor anything typed into the session can be the cause. What remains: a Claude Code Web platform-side provisioning fault, or something specific to this account/organization independent of the repo. MT-CP1001 could not start in any of the three attempts: no auth check, no skill check, nothing pair-specific was exercised. Recorded per MT-CP1005 ("no case is silently skipped... a truncated run is reported as PARTIAL, never as a pass"): `BLOCKED`, not `PARTIAL` — no case body ever ran.

### Notes

- **This path has been attempted three times and none completed.** All three 2026-08-20 attempts stalled at the identical provisioning step before the repository was ever cloned — see the row's Notes for the three variables ruled out (network policy, prompt content, and by construction, repo content). `pair`'s web support therefore remains _assessed, not verified_: no row above carries a `$STORY`/`$PR` pair, so AC1, AC2 and AC7 of the story that authored this path stay unmet. **This is reported to the maintainer as a genuine defect candidate outside pair's own repo (per MT-CP1006) — a Claude Code Web platform issue or an account/org-side block — not something a change to this codebase can fix.** A fourth attempt from a different account/org, if one is available, would isolate "platform-wide" from "this account/org only"; short of that, the finding stands as reported.
- **MT-CP1005 gap surfaced by this run**: none of its five degraded-path rows name "session/container provisioning never completes" as a stop condition — they all assume the container is up and something *inside* it fails (`gh` unauthenticated, MCP unavailable, Playwright unavailable, mid-run interruption, a refused operation). A provisioning stall precedes all of those, and three occurrences with three different variables rule out one-off variance. Add a sixth row for it.
