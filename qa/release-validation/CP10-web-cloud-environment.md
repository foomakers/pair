...
- **Refinement, with an open causation question**: after opening the session in the desktop app, a plain browser tab reload on the same session picked up the completed state — Container cloud ripristinato / Repository aggiornato / Claude Code avviato, all shown correctly. **Not yet isolated: whether the reload alone would have resolved it, or whether opening the app first is what actually unblocked the session.** The app's own UI read "Sessione ripresa" ("session resumed"), which suggests the session may have gone idle and needed an explicit resume action — in which case a reload of an untouched, never-app-opened browser tab might not have been sufficient on its own. The two hypotheses were not tested independently in this run. A clean redo (below) is designed to separate them: on a fresh stuck session, reload the browser tab FIRST, before opening the app at all, and record whether that alone resolves it.
- **Still open**: whether attempts 1 and 2 (different network settings) would also have resolved on a reload, since neither was refreshed before being abandoned — their true state is still unknown. MT-CP1003 (the story end-to-end run) has not yet been attempted from a confirmed-working session.
