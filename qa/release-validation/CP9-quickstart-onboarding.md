...
- **Bootstrap asked no question outside the two still-asked decisions** — no categorization confirmation, no per-section interview, no per-document approval round, no custom-gate question, and **no approval round from any composed skill**: quick mode passes `$approval: auto` and every `assess-*` / `map-*` skill it composes honours it (`skill-conventions/approval-rounds.md`). One signal covers every composed family, so this assertion no longer enumerates them — a newly composed skill that asks is a failure here, not an omission to add to this list. **The one admissible exception** is `/pair-capability-map-contexts` halting on an unbalanced + volatile relationship with no mitigation or acceptance: if that fires, record it and it does NOT fail this assertion — it is a judgement gate, which `auto` never resolves, by design. This assertion is scoped to bootstrap (the `T0 → T1` window); the planning/refinement turns in step 5 are interactive and not covered by it
