...
# (PR #476, the regeneration commit `chore: regenerate mirrors from local dataset`):
# adr-021, adr-022, adr-023 and collaborative-workflow.context.md — four files that
# exist only in the target tree — were regenerated after they had sat drifted on a
# green `main`. So in that region drift accumulates
# undetected until whichever run of this writer comes next, and lands there.
#
# The writer also reads the WHOLE target tree, untracked files included, and the
# registry BEHAVIOUR decides a foreign file's fate — not whether the tree is written:
# under `behavior: "mirror"` (`.pair/knowledge/**`, `.github/agents/**`) a file only
# the target has is DELETED (an untracked `.pair/knowledge/wip-draft.md` does not
# survive the run); under `add` (`.pair/adoption/{product,tech,decision-log}/**`) it is
# kept but INDEXED into `.pair/llms.txt`; under `overwrite` (`.claude/**`) and `skip`
# (`.github/workflows/**`, `.github/ISSUE_TEMPLATE/**`) it comes out byte-identical and
# unindexed. Stash the first two kinds before running (`git stash push -u -- <paths>`);
# `/pair-capability-publish-pr` HALTs on them, and only on them. The unsafe list lives in
# `way-of-working.md` (`mirror-realign-command`); every row is measured in
# `regenerate-mirrors.test.ts`.
#
