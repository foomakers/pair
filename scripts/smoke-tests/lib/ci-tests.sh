...
CI_EXCLUDED=(
  ...
  "remote-source-resolution.sh: OFFLINE_SAFE=false — exercises 'pair install|update --source <https-url>' against a real HTTPS release asset for the happy path (AC2-4); error paths (AC5-7) are offline-deterministic local fixtures, but the scenario as a whole needs network reachability for AC2-4, so it stays out of PR CI"
