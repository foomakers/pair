...
#   4. the 🔴 predicate is INTACT: an identity approval — bot user or App — can never
#      satisfy `human_approval_jq_filter` (AC3, AC6), and
#   5. the guards: no auto-approval path outside the light row, no classification
#      criteria anywhere in the approval flow (D18), grep-verifiable.
#
...
# ==============================================================================
check "identity + approved ⇒ native APPROVE"                 APPROVE \
  "$(identity_verdict_event identity approved 2>/dev/null)"
check "identity + changes-requested ⇒ REQUEST_CHANGES"       REQUEST_CHANGES \
  "$(identity_verdict_event identity changes-requested 2>/dev/null)"
check "identity + no decision ⇒ COMMENT (fail-safe, not APPROVE)" COMMENT \
  "$(identity_verdict_event identity '' 2>/dev/null)"
check "identity + unknown verdict ⇒ COMMENT (fail-safe)"     COMMENT \
  "$(identity_verdict_event identity banana 2>/dev/null)"
check "session mode + approved ⇒ COMMENT (self-approval rejected by the host)" COMMENT \
  "$(identity_verdict_event session approved 2>/dev/null)"
check "session mode + changes-requested ⇒ COMMENT"           COMMENT \
  "$(identity_verdict_event session changes-requested 2>/dev/null)"
check "unknown mode ⇒ COMMENT (never a native verdict on an unresolved actor)" COMMENT \
  "$(identity_verdict_event '' approved 2>/dev/null)"
...
if command -v jq >/dev/null 2>&1; then
  ...
  # A bot-user identity and a GitHub App are both non-`User` accounts on this API.
...
# The predicate text itself must be unchanged by this story.
audit "the 🔴 predicate still excludes non-User accounts" "$EVALUATOR" \
...
# ==============================================================================
# 1. No auto-approval path outside the adoption-gated light row. `light_auto_approve_allowed`
#    is the ONLY definition, and no flow surface may submit an approval unconditionally.
...
for doc in "$REVIEW" "$PUBLISH_PR"; do
  # every APPROVE/auto-approve mention in the flow must sit near the gating helper
  if grep -in 'auto-approv' "$doc" | grep -qvi 'light'; then
    log_fail "$(basename "$(dirname "$doc")") mentions an auto-approval not qualified by 'light'"
    grep -in 'auto-approv' "$doc" | grep -vi 'light'
    FAILED=1
  else
    log_succ "$(basename "$(dirname "$doc")") has no auto-approval path outside the light row"
  fi
done
