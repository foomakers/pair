...
`.claude/skills/**` and `.pair/knowledge/**` copies (`pair update`) in the same commit, or a mirror guard fails later in the
same gate — the dataset copy is inside format scope, its generated twin is not (`.claude/` and root `.pair/` are not
workspace members), and the mirror guards assert each twin equals the OUTPUT of the real `pair update`
transform — never the dataset source itself, which the corpus is transformed away from. `gate:composition` guards the gate against a write-mode step (formatter or eslint autofix)
