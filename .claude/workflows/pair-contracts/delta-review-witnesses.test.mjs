...
test('F-2 (witness): reconstructing the LAST group of a sequential batch would destroy the EARLIER sibling`s work, and the guard does not refuse', () => {
  const { dir } = runDir()
  // g1 owns a file INSIDE the directory g2 owns: restoring `src/shared/` at H0 deletes g1's fix
  const riskId = sequentialBatch(dir, { g1Paths: ['src/shared/util.ts'], g2Paths: ['src/shared/'] })
  firstRepairOf(dir, { phase: 'r1-g2', riskId, head: SHA('3'), reviewPhase: 'r2', reviewedHead: H0, firstFailingHead: H2 })
  const n = nextOf(dir)
  assert.deepEqual(
    { step: n.step, reason: n.reason, refusal: n.refusal },
    { step: 'blocked', reason: 'escalate', refusal: 'reconstruction-overlaps-later-work' },
    `restoring src/shared/ at H0 would delete r1-g1's src/shared/util.ts — a human must decide; instead: ${JSON.stringify({ step: n.step, phase: n.phase, reconstruct: n.reconstruct })}`,
  )
  assert.match(String(n.detail ?? ''), /src\/shared\/util\.ts/, 'the refusal names the work that would be destroyed')
  assert.equal(n.reconstruct, undefined, 'nothing may be restored while the trade is unresolved')
})

test('F-2 (witness): the same defect with IDENTICAL sibling scopes — an earlier sibling of the batch is invisible to the guard whatever the path spelling', () => {
  const { dir } = runDir()
  const riskId = sequentialBatch(dir, { g1Paths: ['src/shared/util.ts'], g2Paths: ['src/shared/util.ts'] })
  firstRepairOf(dir, { phase: 'r1-g2', riskId, head: SHA('3'), reviewPhase: 'r2', reviewedHead: H0, firstFailingHead: H2 })
  const n = nextOf(dir)
  assert.deepEqual(
    { step: n.step, refusal: n.refusal },
    { step: 'blocked', refusal: 'reconstruction-overlaps-later-work' },
    `r1-g1 wrote the very file the reconstruction would restore; got ${JSON.stringify({ step: n.step, phase: n.phase, reconstruct: n.reconstruct })}`,
  )
})

test('F-2 (control): a sibling on genuinely disjoint paths still reconstructs — the guard is not a blanket refusal', () => {
  const { dir } = runDir()
  const riskId = sequentialBatch(dir, { g1Paths: ['docs/readme.md'], g2Paths: ['src/shared/'] })
  firstRepairOf(dir, { phase: 'r1-g2', riskId, head: SHA('3'), reviewPhase: 'r2', reviewedHead: H0, firstFailingHead: H2 })
  const n = nextOf(dir)
  assert.equal(n.step, 'prepare', JSON.stringify({ step: n.step, refusal: n.refusal, detail: n.detail }))
  assert.ok(n.reconstruct, 'nothing overlapping landed: the reconstruction stands')
  assert.deepEqual(n.reconstruct.paths, ['src/shared/'])
  assert.equal(n.reconstruct.fromHead, H0)
})

test('F-2 (control): work that landed BEFORE the behavioural baseline is already inside it — overlapping or not, it never blocks the reconstruction', () => {
  const { dir } = runDir()
  // round 1 writes src/shared/util.ts and its head H1 is the baseline the round-2 regression cites
  redSpec(dir, 'a0', { mode: 'initial' })
  redVerify(dir, 'a0')
  handoff(dir, 'a0', 'implement-phase', { status: 'ok', gatesPassed: true, prNumber: 7, outputHead: H0 })
  review(dir, 'r0', { reviewedHead: H0, verdict: 'CHANGES-REQUESTED', readiness: { ready: false }, findings: [finding('r0-1')] })
  redSpec(dir, 'r1-g1', { plan: { groups: [{ groupId: 'r1-g1', findings: ['r0-1'], owner: 'a', mode: 'behavioral', allowedPaths: ['src/shared/util.ts'] }], carried: [] }, groupId: 'r1-g1', remediationBatchId: 'r1' })
  redVerify(dir, 'r1-g1', { remediationBatchId: 'r1' })
  handoff(dir, 'r1-g1', 'green-fix', { fixed: true, needsHumanDecision: false, outputHead: H1, evidenceLedger: [], remediationBatchId: 'r1' })
  expectDispatch(dir, { step: 'verify', phase: 'r1' }, 'round 1 is reviewed', NO_BUDGET)
  // H1 is reviewed with the AC-7 obligation passing — the baseline the later risk cites
  review(dir, 'r1', { mode: 're-review', reviewedHead: H1, verdict: 'CHANGES-REQUESTED', readiness: { ready: false }, findings: [closed('r0-1'), finding('r1-1', { location: 'src/shared/other.ts:3' })] })
  // round 2: two groups again, the producer owning the directory that CONTAINS round 1's file
  expectDispatch(dir, { step: 'prepare', phase: 'r2-g1' }, 'the next round opens', NO_BUDGET)
  const plan = {
    groups: [
      { groupId: 'r2-g1', findings: ['r1-1'], owner: 'docs', mode: 'behavioral', allowedPaths: ['docs/readme.md'] },
      { groupId: 'r2-g2', findings: ['r1-1'], owner: 'gate', mode: 'behavioral', allowedPaths: ['src/shared/'] },
    ],
    carried: [],
  }
  redSpec(dir, 'r2-g1', { plan, groupId: 'r2-g1', remediationBatchId: 'r2' })
  ...
  expectDispatch(dir, { step: 'prepare', phase: 'r2-g2' }, 'the second group follows', NO_BUDGET)
  redSpec(dir, 'r2-g2', { groupId: 'r2-g2', remediationBatchId: 'r2' })
  redVerify(dir, 'r2-g2', { remediationBatchId: 'r2' })
  handoff(dir, 'r2-g2', 'green-fix', { fixed: true, needsHumanDecision: false, outputHead: SHA('3'), evidenceLedger: [], remediationBatchId: 'r2' })
  expectDispatch(dir, { step: 'verify', phase: 'r2' }, 'round 2 is reviewed on the last head it produced', NO_BUDGET)
  const rr2 = risk({ introducedByRemediationBatchId: 'r2', lastCleanReviewedHead: H1, firstFailingHead: SHA('3') })
  review(dir, 'r2', { mode: 're-review', reviewedHead: SHA('3'), verdict: 'CHANGES-REQUESTED', readiness: { ready: false }, invalidatedBatchId: 'r2', findings: [closed('r1-1'), regressionFinding('r2-9', { regressionRisk: rr2 })] })
  const riskId = JSON.parse(readFileSync(join(dir, 'r2-review-phase.json'), 'utf8')).findings.find(f => f.id === 'r2-9').regressionRisk.riskId
  // first repair of the producer r2-g2, then the regression is observed again
  expectDispatch(dir, { step: 'prepare', phase: 'r2-g2' }, 'the rewind targets the producing group', NO_BUDGET)
  redSpec(dir, 'r2-g2', { groupId: 'r2-g2', remediationBatchId: 'r2', regressionRepairOf: 'r2', regressionGuards: [riskId] }, { attempt: 2 })
  redVerify(dir, 'r2-g2', { remediationBatchId: 'r2', regressionGuards: [riskId] }, { attempt: 2 })
  handoff(dir, 'r2-g2', 'green-fix', { fixed: true, needsHumanDecision: false, outputHead: SHA('4'), evidenceLedger: [], remediationBatchId: 'r2', regressionGuards: [riskId] }, { attempt: 2 })
  expectDispatch(dir, { step: 'verify', phase: 'r3' }, 'the repair is reviewed', NO_BUDGET)
  review(dir, 'r3', { mode: 're-review', reviewedHead: SHA('4'), verdict: 'CHANGES-REQUESTED', readiness: { ready: false }, invalidatedBatchId: 'r2', findings: [closed('r1-1'), regressionFinding('r2-9', { regressionRisk: rr2 })] })
  const n = nextOf(dir, NO_BUDGET)
  assert.equal(n.step, 'prepare', `round 1's work is inside the baseline H1: restoring cannot harm it — ${JSON.stringify({ step: n.step, refusal: n.refusal, detail: n.detail })}`)
  assert.ok(n.reconstruct, 'the reconstruction stands')
  assert.deepEqual(n.reconstruct.paths, ['src/shared/'])
  assert.equal(n.reconstruct.fromHead, H1)
})
