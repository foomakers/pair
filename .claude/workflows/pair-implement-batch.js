...
const lockedRedContract = contract =>
  `LOCKED RED CONTRACT (mandatory): an independent test-only agent has already created and run this failing contract in the SAME worktree: ${JSON.stringify(contract)}. Change implementation/adoption only. Do NOT modify, format, rename, regenerate, delete or weaken any listed test artifact, fixture, test name, expectation, matrix row or its hash. The later verifier recomputes every hash. If the contract is genuinely contradictory, return needsHumanDecision — never repair it by editing the test.`
...
async function driveStory(story) {
  ...
  // The fixer cannot author its own specification. This agent is deliberately limited to RED:
  // it maps the finding to the state owner, writes the finite test matrix and leaves those files
  // dirty for a separate GREEN session. The source fixer is then cryptographically bound to it.
  ...
  while (true) {
    ...
    const preflight = await runPreflight(reviewedHead, prevFindings, [fix.evidenceLedger], redTest, 0)
    ...
    if (firstPreflight.actionable.length) {
      ...
      if (preflightFix.needsHumanDecision)
        return { story, prNumber: pr.prNumber, status: 'failed-preflight', findings: prevFindings, acceptedFindings: accepted, reviewLog }
      const finalPreflight = await runPreflight(preflight.reviewedHead, firstPreflight.actionable, [fix.evidenceLedger, preflightFix.evidenceLedger], preflightRedTest, 1)
