...
- **Every finding is real and evidenced; taste is not a finding.** A reproducer (command or test) or a concrete failure scenario (this input ⇒ this wrong result) — or it is not recorded. On a fresh card's first review, name per AC the test that proves it; a weak or missing test is an ordinary finding. A mutation probe is yours to use when a test looks unable to fail, never a required step.
