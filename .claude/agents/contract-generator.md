...
2. **Read the template** and extract its vocabulary — e.g. the verdict options (the decision values on the `## Verdict` line), the severity levels (the sub-headings under `## Details` → "Findings by severity"), the per-finding fields. Adapt to the template's *current* structure: headings may have moved or been reworded; you are the parser precisely because a deterministic one would break here.
