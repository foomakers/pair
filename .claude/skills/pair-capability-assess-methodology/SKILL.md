...
- **Persistence**: `/pair-process-bootstrap` accepts the proposal and composes `/pair-capability-record-decision(content, target, decision-metadata)` to write the methodology section and record the ADL.
...
- If way-of-working.md doesn't exist, the assessment still runs — the caller creates the file on persist via `/pair-capability-record-decision`.
