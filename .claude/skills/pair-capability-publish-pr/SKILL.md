...
7. **Act — board state:** update the story's board state on the **PM tool** **directly**, in the order the item writer states once and this step applies unchanged — **membership, then a read that confirms it, then the state field** (`/pair-capability-write-issue` Step 7b). Composing `/pair-capability-write-issue` is not the route for a state-only change — its write mode is a **full-body overwrite** and would replace the story's AC/DoD/task breakdown — so the invariant is applied here **by reference**, never re-derived. **Resolve `## State Mapping` first** — the canonical target is `Review`:
   - **A board state maps to `Review`** ⇒ compose with `$status: Review`.
   - **None does** (a minimal board, D4 — a board that reviews on the PR and merges straight to `Done`) ⇒ **omit `$status` entirely** and report `Board: n-a — no Review state on this board`. This is the composed skill's documented skip (its Step 6), and it only stays a skip because the **caller** omits the argument: a `$status` that arrives there has been *requested*, and an unmappable request can only HALT (route (c)). Omitting is therefore not an optimisation — it is what makes the minimal-board path work at all.

   The composition carries its **membership precedes state** invariant with it (its Step 7b: membership → a read that confirms it → the state field), so an item that is not in the tracked view is put there and *confirmed* before the state is written. **Any HALT the composition returns — an unconfirmable membership (Step 7b) or an unmappable requested macrostate (Step 6) — is reported verbatim on the `Board:` row** as `not updated — <reason>` and never absorbed into a green publish; the PR stays ready-for-review either way. If `/pair-capability-write-issue` is not installed or the PM tool is inaccessible, warn and continue — the PR is already ready. PR state itself is never mirrored onto the board.
...
- **No board state maps to `Review`** (a minimal board, D4 — a project that reviews on the PR and merges straight to `Done`): omit `$status` from the step-7 composition and report `Board: n-a — no Review state on this board`. The zero-configuration documented skip, **not** an error and not a degraded publish — the readiness signal is the PR itself.
- **The composed `/pair-capability-write-issue` HALTs on the board write** (membership unconfirmable — its Step 7b; or a requested macrostate no board state can express — its Step 6): report the HALT verbatim on the `Board:` row as `not updated — <reason>` and continue. The PR is published and ready-for-review; a board write that did not happen is **reported, never absorbed into a green publish**, and this skill never HALTs on it (the code-host artifact is the work).
