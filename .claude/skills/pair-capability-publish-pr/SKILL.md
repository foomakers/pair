...
- **way-of-working.md → `## Git Workflow`** — `code-host` (the tool owning branches/PRs) and `base-branch` (default `main`; **a `base-branch` declared under `## Merge Strategy`, where this skill's ≤ 0.4.1 versions documented it, is still honored** — see Phase 2 step 1 for the resolution order, so no existing adoption silently reverts to `main`). **`code-host` absent ⇒ code host = PM tool** (single-tool; the zero-configuration default, not a degradation), and the same tool named in both places is treated exactly as omitted. Resolution, the PM↔code-host routing table, and the cross-linking convention live in one place: [way-of-working / PM-tool + code-host resolution](../../../.pair/knowledge/guidelines/technical-standards/ai-development/skill-conventions/way-of-working-pm-resolution.md) — this skill states only which side each operation is on.
...
1. **Act**: Read **both** git-concerned sections (Adoption Inputs) — `## Merge Strategy` (how the PR ends) and `## Git Workflow` (where the code lives and where the branch starts). Resolve, with defaults for anything omitted:
   ...
   - `base-branch` — the PR target branch. Resolution order: `## Git Workflow` → **`## Merge Strategy` (legacy placement, still honored)** → default `main`. Versions ≤ 0.4.1 of this skill documented the key under `## Merge Strategy`, so an adoption that declared e.g. `base-branch: develop` there keeps targeting `develop`; moving the key to `## Git Workflow` is optional tidying, never a prerequisite.
