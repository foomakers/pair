...
| `$assignee`  | No       | Who the **pull request** is assigned to. Resolved by **the same cascade** `/pair-capability-write-issue` Step 6b applies to an item — argument, then the adoption's `default-assignee`, then none with a warning. Unresolvable ⇒ the PR is still published, unassigned (**never a HALT**). |
