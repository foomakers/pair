...
- **A write the host reports as applied but a read does not show** (a tag, the assignee, ready-for-review — each read back where it is written: tags and assignee in Phase 4 step 3, ready-for-review in step 6): report it as a finding on the corresponding output row and continue. The PR exists and is what matters; what must never happen is reporting the unapplied write as done.
