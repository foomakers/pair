...
├── Identity:   [session (default — no identity configured) | identity: <app|bot-user> | HALTED — configured but unusable]
...
- **A dedicated review identity is configured but unusable** (Phase 5 step 3) — invalid credential, missing permission, unknown health. Report the setup pointer on the `pair-review` row and stop; **never** fall back to the session token. The PR is already created and ready-for-review, so nothing is rolled back — only the check registration and the dispatch do not happen. No identity configured is **not** this case (that is `session`, the default).
