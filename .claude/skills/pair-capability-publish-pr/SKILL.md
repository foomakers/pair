...
7. **Act — board state:** update the story's board state on the **PM tool**, using `/pair-capability-write-issue` (default write mode) when installed. **Resolve `## State Mapping` first** — the canonical target is `Review`:
