...
Deduction inputs, in precedence order: **argument > tags > issue type**. The type decides the level **and** the phase-3 writer; tags decide the orientation and win over the type-derived default. **Most specific row wins**, and the tag row is a **modifier**, not a competing row: it flips the orientation and adds `/pair-capability-map-contexts`, leaving the level and the writer as the base row resolved them.
...
| **no recognized type** on `$root` (fallback)            | asked first (phase 1), exactly as on the no-`$root` path | functional, unless the tag modifier flips it | resolved once the level is answered: broad → `/pair-capability-map-subdomains`, punctual → `/pair-capability-map-contexts` | `/pair-process-plan-stories` with `$epic: <the root's parent epic>` — an untyped root is treated as a **leaf, never a container**, so it enters the writer's registry as an **EXTEND target** exactly as a story root does; no parent epic resolves → **HALT** (no parent to hang the tree from) |
| any type or none + a `tech` / `tech-debt` / `infra` tag (modifier) | unchanged  | technical   | `/pair-capability-map-contexts` — **in addition to, and after**, the base row's capability when that row is broad | unchanged (the base row's)                                                                                                     |
| no `$root` (free theme)                                | asked first (phase 1) | follows the answered level, `functional` default | resolved once the level is answered: broad → `/pair-capability-map-subdomains`, punctual → `/pair-capability-map-contexts` | broad → `/pair-process-plan-epics` with `$initiative: <the initiative the placement points to, confirmed>`; punctual → `/pair-process-plan-stories` with `$epic: <the epic the placement points to, confirmed>` |
...
4. **Act**: Apply the [Orientation Matrix](#orientation-matrix) to the loaded issue's type and tags — no recognized type label resolves on the matrix's **fallback row** (level asked in phase 1, writer `/pair-process-plan-stories` under the root's parent epic), never on no row at all — then state the deduction up-front as a proposal:
   ...
   > Phase 2 will run [`/pair-capability-map-subdomains` | `/pair-capability-map-contexts` | both | resolved once the level is answered]; phase 3 will write with [`/pair-process-plan-epics` | `/pair-process-plan-stories`] under `#[parent ID]` ([the root | the root's parent epic | the initiative the placement points to]).
...
6. **Act**: If the session stops before the interview reaches explicit shared understanding, take grill's handoff offer: the blob-so-far is written to `.pair/working/` so a later `/pair-process-brainstorm` run resumes from it (Step 2 above) rather than restarting.
...
- **No parent to hang the tree from** (Phase 3, **any** root or none) — no initiative exists for a broad free-theme tree, no epic for a punctual one, **or** the `$root` story / untyped root has no parent epic to hang siblings from (an orphan is a normal PM-tool state — sub-issue links are optional); HALT with the `/pair-process-plan-initiatives` / `/pair-process-plan-epics` pointer rather than writing beneath an unrelated parent.
