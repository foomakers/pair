# /pair-process-brainstorm — Parametrization Reference (level · orientation · phase-3 writer)
...
| no `$root` (free theme)                                | asked first (phase 1) | `functional` unless `$orientation` or the tag modifier flips it | resolved once the level is answered: broad → `/pair-capability-map-subdomains`, punctual → `/pair-capability-map-contexts` | broad → `/pair-process-plan-epics` with `$initiative: <the initiative the placement points to, confirmed>`; punctual → `/pair-process-plan-stories` with `$epic: <the epic the placement points to, confirmed>` |
...
The one shape the leaf treatment cannot serve is an untyped root whose **parent is an initiative**: there is no parent epic to hang siblings from, and the root's position already says it is epic-shaped. That case resolves to `/pair-process-plan-epics` under the parent initiative instead of HALTing — the label is missing, the structure is not.

### Level sizes, parent types

On that sub-case the level question is still asked (it is the fallback row), but **the answer cannot change what the tree contains**: the item type follows the resolved parent (`/pair-process-plan-epics` under an initiative ⇒ epics), and the writer is never overridable (see [The triple is a proposal](#the-triple-is-a-proposal)). So answering "punctual — a single story" **sizes the discovery** — how wide the interview goes, how many slices come back — and still yields epic-sized candidates.
