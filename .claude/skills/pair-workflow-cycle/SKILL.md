...
| `$profile`  | No       | Reserved: the execution profile (per-stage `context`, model roles). Unresolvable ⇒ HALT `profile-unresolved`; never a silent default.                                                         |
...
| `codex`  | `spawn_agent`      | `resume_agent` / `send_input` | the agent `.md` body + the skill reference  |
