...
- This keeps the invariant: **only `record-decision` writes adoption files generically** (the sole exception is `/pair-capability-setup-pm`, which writes the PM section of `way-of-working.md` directly); **only assess-* renders** its own adoption content; the caller orchestrates the two.
