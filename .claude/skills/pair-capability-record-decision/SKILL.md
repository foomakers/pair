...
- ADR/ADL: `YYYY-MM-DD-<topic>.md` (today's date, one file per decision).
...
- **Same topic + same date (ADR/ADL)**: If a file `YYYY-MM-DD-<topic>.md` already exists and the developer chose "create new," append an incremented suffix: `YYYY-MM-DD-<topic>-2.md`.
...
- Date-based naming (`YYYY-MM-DD-<topic>.md`) ensures ADR/ADL sortability and avoids numbering conflicts. Sequential naming (`ddr-NNN-<topic>.md`) makes DDR supersede chains and cross-references (e.g., brainstorm conflict flags) easy to cite by number.
