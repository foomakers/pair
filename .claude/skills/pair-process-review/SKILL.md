...
2. **Act — resolve whether an APPROVING review is authorized** (only in `identity` mode, and only when Step 5.2 decided APPROVED). A native `APPROVE` by the identity is what satisfies a host `required_approving_review_count >= 1` — i.e. it makes the PR mergeable with no human action — so it is granted by exactly **one** rule, the adoption-gated light row, and by nothing else:
   - **Synthesize the state once, here**: `resolve_pr_state <gates> <verdict> <tier> <explicit-approval>` from [`pr-state.sh`](../../../.pair/knowledge/assets/pr-state.sh), with the inputs already in hand (`gates` from Step 2.1, the tier from the PR's `risk:*` label via `resolve_tier`, `explicit-approval` = a **non-author human** approval recorded on the current head). **Step 5.4 publishes this same value and does not re-synthesize it** — one call, one answer, so the event and the label can never disagree.
   - **Evaluate the row**: read whether the project's `## Tag Projection` (`tech/risk-matrix.md`) declares the `light` family — a **read of adoption**, not a judgment — then `light_auto_approve_allowed <pr-labels> <light-declared> <tier> <state>`. Exit 0 ⇒ `approve-authorized = 1`; anything else ⇒ `0`, and keep the condition the helper named on stderr for the report. The four conditions and their containments are Step 5.4b's.
   - In `session` mode skip this entirely: `approve-authorized = 0`. There is no principal whose approving review the host would accept anyway.
3. **Act — map the verdict onto the host event**: `identity_verdict_event <mode> <verdict> <approve-authorized>` (never hand-rolled here):
   ...
   - **`identity` + APPROVED + `approve-authorized = 1`** ⇒ `event = APPROVE`. This **is** the light row's approving review; it is the only path in this flow to a native APPROVE.
   - **`identity` + APPROVED + not authorized** ⇒ `event = COMMENT`, with the verdict token (APPROVED) leading the body. The judgment is published in full and the `pair-review` check still carries `success` (Step 5.4), so the PR reaches `ready-to-merge` exactly as before — what the identity does **not** do is sign the host's required approval on the project's behalf. Report it as `Identity approve: not authorized — <unmet condition>`.
...
4. **Act**: Submit the native review on the code host with that event (for GitHub, per [github-implementation.md](../../../.pair/knowledge/guidelines/collaboration/project-management-tool/github-implementation.md); another host's implementation guide supplies the equivalent commands), passing the compiled verdict-first report as the review **body**:
...
- **Adoption declares no `light` projection** (Step 5.3 step 2 — the default everywhere): the row authorizes nothing, so an approving verdict is submitted as a COMMENT-form review; report `Light row: not-authorized — adoption declares no light family` and continue. A `light` label present without the declaration changes nothing. Does NOT HALT.
