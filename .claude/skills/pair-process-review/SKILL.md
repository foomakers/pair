...
The compiled report **is the body of the native review on the code host** — the verdict is the review action; there is **no separate PR comment** (decision Q5).
...
1. **Act — resolve the identity mode**: read `Review identity` from [way-of-working.md](../../../.pair/adoption/tech/way-of-working.md) (`none` is the **default**; `app` / `bot-user` name a configured identity), source the shipped [`review-identity.sh`](../../../.pair/knowledge/assets/review-identity.sh) and call `resolve_identity_mode <configured> <healthy>` — `healthy` is 1 only when the identity's credential authenticated **and** the required permissions were observed (the host guide lists the probes). Three outcomes, and no fourth:
...
2. **Act — map the verdict onto the host event**: `identity_verdict_event <mode> <verdict>` (never hand-rolled here):
   - **`identity` + APPROVED** ⇒ `event = APPROVE`; **`identity` + CHANGES-REQUESTED** ⇒ `event = REQUEST_CHANGES`. The identity is **not** the PR author, so the host accepts the real event — this is what retires the `--comment` degradation when an identity exists.
   - **`session`** (or any unresolved verdict) ⇒ `event = COMMENT`. The reviewer is the author there and GitHub rejects a self-authored `APPROVE` / `REQUEST_CHANGES`; the verdict token (APPROVED / CHANGES-REQUESTED) still leads the body, so the decision and full report are recorded, never lost. This form stays fully supported — the identity **upgrades** it, it does not replace it. See Graceful Degradation.
3. **Act**: Submit the native review on the code host with that event (for GitHub, per [github-implementation.md](../../../.pair/knowledge/guidelines/collaboration/project-management-tool/github-implementation.md); another host's implementation guide supplies the equivalent commands), passing the compiled verdict-first report as the review **body**:
...
4. **Act**: On re-review, submit a **fresh** native review — both documented paths append (MCP `create`; `gh pr review` CLI), neither edits a submitted body. GitHub's latest-review-governs semantics mean the newest review carries the verdict while earlier reviews stay as visible history, so re-invocation is safe without editing in place (idempotency).
5. **Verify**: The native review is submitted with the verdict-first body, **and a read of the PR's reviews shows which account authored it** — the identity when one was resolved, the session account otherwise. Report the resolved mode on the `Identity:` output row; never claim per-identity attribution the read does not show. No separate review-comment artifact exists.
...
### Step 5.4b: Adoption-Gated Light Auto-Approval (inert unless declared) & Audit

One row, and it does nothing on a project that has not opted in. It is evaluated **after** the synthesis (Step 5.4), consumes it, and never re-decides it — `resolve_pr_state` is unchanged. Model: [pr-states.md](../../../.pair/knowledge/guidelines/collaboration/project-management-tool/pr-states.md) § Adoption-gated light auto-approval.

1. **Check**: Did Step 5.3 resolve `identity` mode? **Skip** the whole step in `session` mode — with no identity there is no principal that could sign an approving review the host would accept, so there is nothing to gate.
2. **Act — read the declaration**: does the project's `## Tag Projection` (`tech/risk-matrix.md`) declare the `light` family? This is a **read of adoption**, not a judgment: the declaration is the gate, and a hand-applied `light` label on a project that declares nothing triggers nothing.
3. **Act — evaluate**: `light_auto_approve_allowed <pr-labels> <light-declared> <tier> <state>` (from `pr-state.sh`) — the four conditions are all evaluated there, so nothing is decided in this step: adoption declares `light`, the PR carries the `light` tag, the tier is **below 🔴**, and the state is already `ready-to-merge`. **No classification criteria** are read here or there — tags, the declaration, the gate result and the verdict, nothing else (D18).
4. **Act — approve, or no-op**: on approve, the identity submits the native approving review that satisfies the host's required-approvals rule, so the PR is mergeable with no human action. On no-op, do nothing and report the unmet condition the helper named. **There is no auto-approval path outside this light row** — grep-verifiable.
5. **Act — audit (every identity approval AND every identity block)**: post the comment `identity_audit_comment <action> <tag> <declared> <tier> <state>` renders, so the reason is reconstructable from the PR alone: which tag, which declaration, which tier, which state. The audit is not optional and not summarized from memory — it is the adapter's projection of the same inputs.
6. **Verify**: A read of the PR shows either the identity's approving review **plus** its audit comment, or no approval at all and a recorded reason. The `pr-state:*` label from Step 5.4 is unchanged by this step (label mechanics are Step 5.4's, untouched). **At 🔴 nothing here ever fires**, and even when it does the identity's approval does **not** satisfy `pair-explicit-approval` — that predicate requires a human account (`user.type == "User"`), so the 🔴 gate stands whatever this row did.
...
├── Light row:  [n-a — no identity | n-a — adoption declares no light projection | no-op — <unmet condition> | approved by identity + audit comment posted]
...
- **A dedicated review identity is configured but unusable** (Step 5.3 — invalid credential, missing permission, unknown health): `resolve_identity_mode` returns `halt`. Report the adapter's setup pointer (the host guide's _Dedicated review identity_ section) and stop **before any host write**. Never fall back to the session user — misattributing a review to the human whose token is loaded is the failure this HALT exists to prevent. No identity configured at all is **not** this case: that is `session` mode and it proceeds normally.
...
- **No dedicated review identity configured** (Step 5.3): `session` mode — the session token writes, the verdict is the `--comment` form, `pair-review` is a commit status. The **zero-configuration default**, reported as `Identity: session`, never as a degradation. Does NOT HALT.
...
- **Adoption declares no `light` projection** (Step 5.4b — the default everywhere): the light row is inert; report `Light row: n-a` and continue. A `light` label present without the declaration changes nothing. Does NOT HALT.
...
- **Who decides and who acts are different questions.** The verdict is this skill's judgment; the credential that writes it to the host is resolved by the shipped [`review-identity.sh`](../../../.pair/knowledge/assets/review-identity.sh) (Step 5.3) — `session` by default, `identity` when a project provisions one, `halt` when one is configured but broken. Adopting an identity **never** relaxes the 🔴 rule: `human_approval_jq_filter` requires `user.type == "User"`, so the identity's approval cannot satisfy `pair-explicit-approval` and a `risk:red` PR still needs a second human account (ADR-018, amendment 2026-08-28).
- **The light row is the only auto-approval path, and it is off unless declared** (Step 5.4b): `light_auto_approve_allowed` reads a tag, an adoption declaration, the tier and the synthesis — **zero classification criteria** (D18) — and `resolve_pr_state`'s table is untouched by it.
