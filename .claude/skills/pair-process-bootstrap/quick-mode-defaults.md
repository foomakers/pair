...
| Step 2.2     | which `assess-*` skills run, and how                                 | installed ones are composed **with** their quick signal (Path A `$choice`), never plain — the family's declared default is guided (Path C). Project state fills `$choice` where it can; otherwise the per-type fallback row below does | project state     |
...
| Step 2.3     | tech stack section                                                   | read from project state (`package.json`, lockfiles, config files); **still asked** on an empty repo — the fallback table has no stack row by design | project state     |
