...
- Checkpoints complement, not replace, git/PM-tool state. When state can already be reconstructed reliably from git and the PM tool (as `/pair-process-implement` does today), a checkpoint file adds an explicit, fast-to-read summary — most valuable across context resets and subagent handoffs, where reconstruction from scratch is expensive or impossible.
