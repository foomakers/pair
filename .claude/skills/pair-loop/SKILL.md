...
3. **Act — any other tool** (no in-harness fan-out runner): take the **one-card path** below — never iterate multiple cards in this session's own context. Whether that single card is the WHOLE run (tier 3, degraded) or one iteration of an unattended run (tier 2) is the CALLER's business, not this skill's: an external driver re-invokes on the continue-token this step prints, in a fresh process each time, so the steps here are identical either way. Say which tier the invocation is in the output only when the caller told you (e.g. `pair-cli run` states its own resolution):
   ...
   - If the resulting PR is review-approved, the card's tier is `## Eligibility`'s own value (the only tier `## Auto-Advance` may ever name), re-read the card's `risk:*` label right now (a mid-run raise parks the card even with an approved PR), re-read the PR signals on the code host (current remote head SHA, `pair-review` conclusion, `pair-explicit-approval` conclusion on that head — anything unreadable parks the card), and merge ONLY when all hold: tier unchanged and in `## Auto-Advance`, remote head identical to the reviewed head, both conclusions `success` (the latter is D10: at 🔴 without a recorded human approval it cannot be success), and the tier's gate set verifies green (`/pair-capability-verify-quality`). That conjunction IS `merge_allowed` on fresh signals — the loop executes merges the synthesis authorizes, never ones it does not. On ALL green, merge unattended (adopted strategy, squash default; commit message per the commit template), then close the story and cascade parents with board-state `Done` confirmed by read, delete the branch and remove the checkpoint. **Otherwise** — tier moved, head moved, a conclusion not `success`, or the gate came back red — do NOT merge, park the card naming which item failed, AND post a comment on the card's issue recording that it awaits action — the same "on the issue," not audit-only, guarantee the fan-out path gives.
