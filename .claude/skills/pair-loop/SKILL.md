...
2. **Act — Claude Code**: Delegate the entire unattended run to the `pair-loop` workflow (distributed alongside `pair-implement-batch` in the `workflows` registry), passing `{ policyText, root, overrides, predicateOverride, startIteration }` — the resolved policy text, `--root` (if given), any run-argument overrides (pin-sequential, exclude), `--predicate` as `predicateOverride` (the Argument tier of the cascade, replacing the adoption file's `## Stop Predicate` for this invocation only), and `--iteration` as `startIteration` (resuming the counter from a continue-token rather than restarting at 0). The workflow validates every one of these by type and content before touching any card — `root` and the predicate reach agent prompts that run `gh`. It then drives every iteration — selection, dependency + mutex analysis, batch composition, `implement-batch` composition, auto-advance (re-reading each card's tier immediately before merging — a mid-run tier raise blocks it even with an approved PR), audit (write verified, never assumed), stop-predicate evaluation — in fresh per-card subagents (ADR-017 §3, architectural invariant: this orchestrator's context never grows with the number of cards). An escalated or failed card is excluded from every later iteration in the same run, never silently re-driven. Report the workflow's returned iteration count and audit summary when it finishes or is interrupted.
3. **Act — any other tool** (no fan-out runner): take the **degraded one-card path** — never iterate multiple cards in this session's own context:
   ...
   - If the resulting PR is review-approved, the card's tier is in `## Auto-Advance`, and the 🟢 gate set verifies green (`/pair-capability-verify-quality`), push and merge unattended; otherwise leave it for the human and say so.
