...
**Act**: Run the eligibility filter, the dependency analysis and the mutex analysis exactly as Step 1 defines them. Before asking `cap`, use the `harnessCeiling` the bind step returned; when it is `null` on a `spawn-wait` binding, read the harness configuration key in that same binding's `concurrencyKey` and pass the non-negative integer it returns. If the key cannot yield a usable ceiling, **HALT this fan-out path** instead of silently omitting the harness limit. Then ask `cap` for the effective parallelism, passing that ceiling, the dependency-allowed count and `## Max Parallelism` as resolved in Step 0. **Print the returned line** — it names the cap and which of the three limits bound it. A cap of `0` dispatches nothing this iteration. An unknown key inside `ceilings` is rejected too: a dropped harness ceiling dispatches the policy's parallelism into a harness that allows less, the surplus subagents come back dead and are charged to their cards as failed phases, and the printed line names the wrong binding limit.
...
3. **Act**: Wait with the bound wait handle, **always under an explicit timeout** — and pass `waitTimeoutMs` exactly as the `bind` step returned it. For a `spawn-wait` binding it is always a number, so there is no branch to take and nothing to fill in: `waitTimeoutSource` says whether the session reported it (`probe`) or the realization declares it (`realization-default`), and `waitTimeoutKeys`, when it is not `null`, names the config a project raises to change it. An unbounded wait is not permitted — it is how an unattended run hangs forever on a dead subagent — and neither is a wait bounded by a number you chose: inventing a timeout, or omitting the argument and trusting an unseen default, is the inference the probe convention exists to keep out.
...
Record the decision on the review's audit record (`action`, `round`). It is not a convention you may skip: the `audit` command **refuses** a record that completes a `review` without an `action` of `converged`, `fix` or `escalate`, and the resume reconstruction closes the cycle on `converged` alone — anything else, missing included, leaves it open and re-enters the card at `review`. Without both halves, a run killed between a `fix` decision and its fix dispatch resumed with nothing left to do and carried an unfixed PR to the merge gate.
...
**Act**: Append every dispatch and every outcome to the audit through the asset's `audit` command, which writes and reads the file back. Every record carries this invocation's non-empty `run` id. A record about a **card** also names it (`id`); a record about the **run** — the realization announcement from Step 1.1, a run-level halt — is written `{"kind":"run", …}` and needs no id. The command refuses a blank/missing run and anything that is neither card nor run, so the three ways of getting it wrong are all loud: an unscoped escalation could be treated as another run's history, a record with no id was silently unreadable on resume, and one with an invented id came back as a phantom card owed a full pipeline. If the write fails, **HALT the run** — an unattended run with no audit trail is not an acceptable degraded mode. Per-card resumable state stays `/pair-capability-checkpoint`'s; there is no third store, and `.pair/working/` is never loaded as ambient context.
...
- **Never applies a bound it invented.** The wait timeout comes from the binding — the probed value, or the realization's own declared fallback, with the source named either way. A number the run chose for itself is the same inference the probe rule forbids, and "nothing was readable" is not an escape: the binding always resolves to one.
