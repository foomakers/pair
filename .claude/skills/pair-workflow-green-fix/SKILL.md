...
4. Re-invoke `/pair-capability-publish-pr` (create-or-update): it rewrites the PR body to describe the CURRENT head — never append a round-by-round history — and keeps tags and `pr-state:*` in sync. It will emit `Review: review-dispatch-required`; that is expected, the coordinator drives the re-review.
