...
- **Persistence**: `/pair-process-bootstrap` accepts the proposal and composes `/pair-capability-record-decision(content, target, decision-metadata)` to write the observability section and record the ADL.
...
- If infrastructure.md doesn't exist, the assessment still runs — the caller creates the file on persist via `/pair-capability-record-decision`.
