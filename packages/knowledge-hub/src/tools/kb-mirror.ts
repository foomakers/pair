 ...
 *
 * That is not a theory about the corpus, it is its measured shape: 440 of the
 * 441 KB markdown files already differ from their dataset source through
 * exactly this transform. The single byte-identical one
 * (`guidelines/collaboration/templates/code-review-template.md`) was the
 * DEFECT — it documented `/assess-security`, a command no reader's assistant
 * exposes — and #228's `expect(mirror).toBe(dataset)` had frozen that defect
 * into an invariant, so every `pair update` run reintroduced a red test.
 *
...
// Both borrowed from the sibling mirror module rather than duplicated: despite
// the `Skills`/`SkillMd` in their names (the context they were introduced in),
// `readSkillsDatasetFromDisk` is a generic recursive "directory → posix-keyed
// content map" reader and `diffSkillMd` a generic compact line-diff. Reusing
// them keeps ONE drift-report format across both mirror guards.
import { readSkillsDatasetFromDisk, diffSkillMd } from './skill-md-mirror'
...
/**
 * Every markdown file the KB dataset contributes, as sorted posix paths relative
 * to `datasetKbDir`. Derived from the dataset at collection time and recursive,
 * so a newly added KB file is guarded with no test edit and nothing anywhere
 * encodes HOW MANY files exist.
 *
 * Markdown-only for the same reason as the skills mirror: only markdown goes
 * through the content rewrites this guard exists to pin.
 */
export function kbDatasetMarkdownPaths(datasetKbDir: string): string[] {
  return Object.keys(readSkillsDatasetFromDisk(datasetKbDir))
