...
describe('assess-coupling catalog registration (#263)', () => {
  ...
  for (const [label, next, guide] of [
    ...
    it(`${label} next catalog lists assess-coupling and states 32 capability / 50 total`, () => {
      expect(next).toMatch(/assess-coupling/)
      expect(next).toContain('32 capability')
      expect(next).toContain('50 skills')
    })

    it(`${label} skills-guide lists assess-coupling and states 32 capability / 50 total`, () => {
      ...
      // count was asserted (the guide states it as "Total: 50").
