...
describe('publish-pr realigns mirrors before its gate (#419)', () => {
  ...
  it('HALTs before the run over untracked files under the UNSAFE trees — not every tree the command writes into', () => {
    ...
    //
    // Round-7 finding (Minor): round 6 scoped that check to "the trees the command writes into",
    // and the command ALSO writes into trees whose behaviour neither deletes nor indexes a foreign
    // file (`.claude/**` is `overwrite`; `.github/{workflows,ISSUE_TEMPLATE,...}` is `skip`).
    // MEASURED: untracked `.claude/settings.local.json` and `.github/workflows/wip.yml` survive
    // the run byte-identical, entry-identical and unindexed — so the round-6 scope HALTed every
    // publish for a loss that does not happen. The scope is now the UNSAFE set the adoption names
    // (deleted or indexed), and the skill says in so many words that a written tree is not enough.
    // Every row: regenerate-mirrors.test.ts, 'leaves an untracked file under an overwrite or skip
    // tree byte-identical and unindexed'.
    const c = dataset()
    const p1 = phase1()
    ...
    // The check is scoped by the UNSAFE trees the adoption names, and skipped when it names none.
    expect(c).toMatch(/the unsafe trees the adoption names/)
    expect(c).toMatch(/names no unsafe trees[\s\S]{0,300}skipped/)
    // ...and the skill itself rules the written-but-harmless trees OUT of that set, by behaviour.
    expect(c).not.toMatch(/under the trees the command writes into/)
    expect(p1).toMatch(/`overwrite` or `skip`[\s\S]{0,400}byte-identical/)
    expect(p1).toMatch(/not (in|part of) (that|the) set/)
    // A HALT condition, listed with the others.
    expect(c).toMatch(/\*\*Untracked files under the unsafe trees\*\* \(Phase 1\)/)
  })
...
describe("this repository's own wiring for the realignment (#419)", () => {
  ...
  it('names the UNSAFE trees explicitly — deleted under mirror, indexed under add — and rules the overwrite/skip trees out', () => {
    ...
    // and an untracked knowledge file is deleted. Round-7 finding (Minor): the HALT was scoped to
    // that "where the output lands" list, which also holds `.claude/**` (overwrite) and
    // `.github/workflows/**` (skip) — trees the run leaves a foreign file byte-identical and
    // unindexed under. The bullet now names the unsafe set and the safe set separately; every
    // row is measured in regenerate-mirrors.test.ts against the real script and the real
    // registry config (apps/pair-cli/config.json + llms-generation.ts).
    const wow = readFileSync(WAY_OF_WORKING, 'utf-8')
    const gates = sectionBetween(wow, '## Quality Gates', '### Review Tier Matrix')
    expect(gates).toMatch(/untracked[\s\S]{0,400}`\.pair\/llms\.txt`/)
    expect(gates).toMatch(/behavior: "mirror"/)
    expect(gates).toMatch(/behavior: "add"/)
    expect(gates).toMatch(/git stash push -u/)
    // The unsafe set, by tree and by fate.
    const unsafe = /\*\*Unsafe[^*]*\*\*([\s\S]*?)\*\*Safe/.exec(gates)
    expect(unsafe).not.toBeNull()
    const unsafeText = unsafe?.[1] ?? ''
    expect(unsafeText).toContain('`.pair/knowledge/**`')
    expect(unsafeText).toContain('`.github/agents/**`')
    expect(unsafeText).toMatch(/\*\*deleted\*\*/)
    expect(unsafeText).toContain('`.pair/adoption/product/**`')
    expect(unsafeText).toContain('`.pair/adoption/tech/**`')
    expect(unsafeText).toContain('`.pair/adoption/decision-log/**`')
    expect(unsafeText).toMatch(/\*\*survives but is indexed\*\*/)
    expect(unsafeText).not.toContain('`.claude/**`')
    // The safe set: written into, harmless — named so the HALT cannot be read onto them again.
    const safe = /\*\*Safe[^*]*\*\*([\s\S]*?)`git stash push -u/.exec(gates)
    expect(safe).not.toBeNull()
    const safeText = safe?.[1] ?? ''
    expect(safeText).toContain('`.claude/**`')
    expect(safeText).toContain('`.claude/settings.local.json`')
    expect(safeText).toContain('`.github/workflows/**`')
    expect(safeText).toContain('`.github/ISSUE_TEMPLATE/**`')
    expect(safeText).toMatch(/behavior: "overwrite"/)
    expect(safeText).toMatch(/`skip`/)
  })
