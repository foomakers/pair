...
# /manage-flags — Feature Flag Lifecycle Manager
...
- **Input**: `/implement` may invoke `/manage-flags` with `$action=create` when a task requires feature-flagged code.
- **Output**: Returns flag creation summary. `/implement` continues with the flag available.
