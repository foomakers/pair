...
- **Output**: Returns the debt assessment report. /review incorporates findings into review output.
...
- This skill is **read-only** when detecting — it does not modify code. Decision recording via `/record-decision` is the only write action.
...
- **Read-only for detection** — this skill inspects code but never modifies files directly. The only write action is decision recording via `/record-decision`.
