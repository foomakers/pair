---
name: assess-debt
description: "Assesses technical debt using resolution cascade (Argument > Adoption > Assessment). Categorizes debt (code, design, test, documentation, infrastructure), applies prioritization formula (impact x effort), proposes remediation priority. Idempotent: detects existing assessment; `$mode: scan` converts findings into tracked tech-debt items keyed by location+pattern to avoid duplicates. Invocable independently or composed by /review."
version: 0.4.1
author: Foomakers
---
...
| `$mode`   | No       | `assess` (default) — categorize/prioritize/report only. `scan` — grep-based detection (code smells + design-rule violations) that creates tracked `tech-debt` items via `/write-issue`. See [Scan Mode Algorithm](#scan-mode-algorithm-mode-scan). |
...
| `/write-issue`      | Capability | No — only in `$mode: scan`, or when `/review` flags new debt introduced by a PR |
...
3. **Act**: Compose `/record-decision` for each decision-worthy item:
...
1. **Act**: Scan the target (full codebase, or PR diff when composed by `/review`) for:
...
1. **Act**: For each remaining group, compose `/write-issue` with `$type: task`, `$content` containing: title, description (matched locations + remediation note), the detection key(s) covered (so future scans can match Step S2), and label `tech-debt` in addition to the type label. Map **Severity → Priority**: High → P1, Medium → P2, Low → P3.
...
3. **Skip**: If `/write-issue` is not installed → report candidate items without creating them (see Graceful Degradation).
...
├── Created:        [N tech-debt items | 0 — /write-issue not installed]
└── Severity:       High: [N] | Medium: [N] | Low: [N]
...
- **Input**: `/review` invokes `/assess-debt` during the completeness phase (Phase 4), scoped to the PR diff (default `assess` mode — `$mode: scan` is not invoked automatically by review; it is a separate, explicit invocation for full-codebase sweeps).
- **Output**: Returns the debt assessment report, distinguishing items **introduced by this PR** from pre-existing debt surfaced incidentally.
  - For each item introduced by the PR: `/review` composes `/write-issue` (`$type: task`, label `tech-debt`, priority from Severity → Priority mapping) to create a tracked item. Re-invocation on the same PR head commit does not recreate items already created earlier in the session (idempotent, per detection key).
...
- This skill is **read-only** when detecting in `assess` mode — it does not modify code. Decision recording via `/record-decision` and item creation via `/write-issue` (scan mode, or PR-introduced debt) are the only write actions.
...
- If `/write-issue` is not installed, `$mode: scan` reports candidate items without creating them — warn: "`/write-issue` not installed — scan results are informational only."
...
- **Read-only for detection** — this skill inspects code but never modifies files directly. Writes happen only via `/record-decision` (decisions) and `/write-issue` (scan-mode items, or PR-introduced debt when composed by `/review`).
...
- Debt never blocks PRs (R7.2) — this applies uniformly whether debt is found via `/review` composition or a standalone `$mode: scan` sweep.
