...
- **review** — evidence is the **code/diff** (observed footprint, `/assess-security` verdict, `/assess-coupling` verdict). The review value is a **floor**: it confirms or **raises** the refinement tier, and **never lowers** it (quality-model §3.2, D17).

## Arguments

| Argument   | Required | Description                                                                                                                             |
| ---------- | -------- | --------------------------------------------------------------------------------------------------------------------------------------- |
| `$context` | No       | `refinement` — classify from the story context. `review` — classify from the diff. Auto-detected: called by `/refine-story` → `refinement`; by `/review` → `review`; a PR/diff in context → `review`; otherwise → `refinement`. |
...
3. **Act**: Auto-detect — invoked by `/refine-story` → `refinement`; by `/review`, or with a PR/diff in context → `review`; otherwise → `refinement`.
...
- **review** — from the diff: observed footprint (change/diff risk), the `/assess-security` review verdict (security relevance, quality-model §3.1), and the `/assess-coupling` verdict on the diff (coupling balance). An unreadable diff (huge/binary) falls back to file-path/service-level evidence and flags **low confidence** in the `<details>`.
...
3. **Act**: If no `## Tag Projection` section, **propose only the `risk` default** (propose-then-write-if-confirmed, the pattern `/verify-quality` uses for its first-run Custom Gate Registry):
   ...
   On **yes**, write the `## Tag Projection` → `Active: risk` section into `tech/risk-matrix.md`; on **no**, write `Active: none`. This registry write is `classify`'s own (the §5 precedent — the same self-write `/verify-quality` performs for its gate registry), not adoption **decision** content, so it does not route through `/record-decision`.
...
**Composed by `/refine-story`** (refinement): invoked with `$context: refinement` after the technical analysis. Returns the matrix; `/refine-story` embeds the 1-line + `<details>` into the story body (D22) and lets `classify` apply/propose tags. Absent ⇒ `/refine-story` warns and continues without a matrix (graceful degradation — never HALTs on classify's absence).

**Composed by `/review`** (review, Phase 1): invoked with `$context: review` against the PR diff. Produces the review-time matrix (confirm-or-raise, never lower). The `/assess-security` verdict (Phase 2) feeds the **Security relevance** dimension and the `/assess-coupling` verdict feeds **Coupling balance** — both raise-only, consistent with the review floor. Any raise to `risk:red` is surfaced for `/review`'s own decision step; `classify` never blocks or merges — it has no such authority.
...
- **`/assess-security` not available** (review): fall back to the path-heuristic security relevance (quality-model §3.1) instead of the verdict; note the fallback.
- **`/assess-coupling` not installed / no DDD artifacts**: coupling dimension "not assessed" (never a HALT).
- **`/refine-story` or `/review` absent as caller**: run standalone via direct invocation.
