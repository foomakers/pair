...
- **Persistence**: `/bootstrap` accepts the proposal and composes `/record-decision(content, target, decision-metadata)` to write the Testing section and record the ADL, then includes those changes in the next commit.
...
- If tech-stack.md doesn't exist, the assessment still runs and produces its proposal — the caller creates the file on persist via `/record-decision`.
...
- **Section ownership**: this skill renders content ONLY for the Testing section of tech-stack.md. /assess-stack owns core sections, /assess-ai owns AI section. The single adoption writer is `/record-decision`.
