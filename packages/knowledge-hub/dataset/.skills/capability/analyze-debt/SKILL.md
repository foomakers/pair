...
2. **Act**: If a High-severity item is worth scheduling, recommend that the developer promote it **deliberately** to the backlog via `/write-issue` (`$type` per template, `tech-debt` label). This is always a manual, selective decision made by the developer.
...
- **Output**: Returns the debt report. /review incorporates findings into review output (the Tech Debt section).
  ...
  - Items worth tracking are promoted deliberately (after review) via `/write-issue` with the `tech-debt` label — /review itself only reports them.
