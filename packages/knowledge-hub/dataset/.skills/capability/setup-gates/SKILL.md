...
2. **Act**: Generate the **pre-merge pipeline** appropriate for the adopted stack and hosting. **Which pipeline depends on `TIERING`:**
   ...
   - **Coverage commit-back workflow (nested opt-in) — a SEPARATE, push-triggered workflow, never a step in the gate**: emit the [commit-back workflow](../../../.pair/knowledge/guidelines/infrastructure/cicd-strategy/tier-aware-pipeline.md#coverage-baseline-commit-back-nested-opt-in-its-own-post-merge-workflow-never-a-gate-job) **only when `COMMIT_BACK = enabled`** (Step 3.5) — which, being nested, can only happen where a coverage job is emitted too. **Do not put the commit-back in the pre-merge pipeline**: that pipeline is triggered by `pull_request`, and the commit-back writes only on a **push to the base branch**, so a step placed there would print `SKIPPED (not-base-push)` on every run forever — an opt-in that is on and does nothing, which is worse than one that is off. Adding `push:` to the gate is not the fix either: it resolves its tier from the PR's `risk:*` label, so every merge would fail-safe to 🔴 and re-run the full suite against the guideline's own "no gate re-run post-merge" rule. Emit the workflow as that guideline writes it: `on: push: branches: [<the project's base branch>]`, it measures its own coverage (the gate's numbers were produced on a different tree and are not available post-merge), then invokes the shipped **KB asset** `node .pair/knowledge/assets/coverage-ratchet.cjs` — generated from pair's tested module and installed with the KB, so no version pin and no npm registry round-trip in the adopter's pipeline (ADR-023). **Substitute the base branch from the adoption** (`## Git Workflow` → `base-branch`, default `main`) in both the trigger and `--base-branch`; never leave the placeholder and never hardcode `main` for a project that declared otherwise. **Substitute the measured types too, from the same source the `coverage` job reads**: emit one `<type>=<pct>` entry per `baseline.<type>` the project's `tech/coverage-baseline.md` declares, each from its own report path — a single hardcoded `default=` makes the ratchet answer "no valid committed baseline.default" on every push, so a repo with per-type baselines would never raise one. Where nothing was measured, the step is skipped rather than invoked with an empty list (an empty `--measured` is a malformed invocation and exits non-zero by design). It is **never a required check** and it always exits 0: persistence is a side effect that may fail alone, never a verdict. With `COMMIT_BACK` `disabled` or never asked (the default), emit **nothing** — no workflow, no credential reference anywhere — and the generated pipeline is byte-identical to one generated before this question existed.
...
4. **Verify**: Configuration files written, including the pre-merge pipeline (full-suite by default; tag-only tier-aware resolution + tier-scoped jobs only when `TIERING = enabled`) + required-check wiring, the post-merge staging (build+deploy only) config, the secret-scanning job, and `.gitleaks.toml`. If tiering is enabled, grep the generated pipeline for classification tokens (schema/diff/path heuristics) — there must be none (D18). If disabled, confirm no `risk:*` resolution was generated and every suite job runs unconditionally. If `COMMIT_BACK` is anything but `enabled`, grep **every** generated workflow for `coverage-ratchet` and `COVERAGE_RATCHET_TOKEN` — there must be no match, and no credential in any step's environment. If it **is** `enabled`, verify the two properties that decide whether it can ever work: the commit-back lives in a workflow whose trigger is `push` on the **resolved base branch** (never in the `pull_request` gate), and no `<placeholder>` survived the substitution.
...
├── Coverage commit-back: [disabled (default — nothing emitted) | not asked (guardrail off) | enabled (opt-in — push-triggered workflow emitted on <base-branch>; COVERAGE_RATCHET_TOKEN to provision)]
...
- **Automating the raise is a nested opt-in, and it is the one flag that must not be offered on its own** (Step 3.5): `Coverage baseline commit-back` is asked only where `Coverage guardrail` is `enabled`, recorded as a nested bullet, and emitted as a separate workflow only when it is on — a baseline ratcheted for a gate that never runs would raise a floor nothing checks. What is emitted is a **separate push-triggered workflow**, not a step of the pre-merge gate — a commit-back inside a `pull_request` pipeline can never write, so emitting it there would be the very silent opt-in this question exists to remove. It invokes the shipped KB asset `node .pair/knowledge/assets/coverage-ratchet.cjs` (ADR-023), so what a project's pipeline runs is the same implementation this corpus documents rather than a copy of it. It needs a credential **the adopter provisions** (`COVERAGE_RATCHET_TOKEN`: `contents: write` + `pull requests: write`, no protection bypass); without it the step warns and the coverage verdict is unchanged. Default stays `disabled` — this question makes enabling it work, it turns it on for nobody.
