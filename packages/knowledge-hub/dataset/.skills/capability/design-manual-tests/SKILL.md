---
name: design-manual-tests
description: "Designs a manual test suite — critical path files + suite README — for post-release validation, by analyzing released artifacts, architecture, and deployment targets. Not for planning the automated-testing approach (use /assess-testing). Prerequisite for /execute-manual-tests."
version: 0.4.1
author: Foomakers
---

# /design-manual-tests — Manual Test Suite Designer
...
- **Input**: `/execute-manual-tests` detects no suite → suggests running `/design-manual-tests`.
- **Output**: Returns the suite directory path. `/execute-manual-tests` can then re-invoke with `$suite` pointing to the generated directory.

When invoked **independently**:

- Generates the full suite and presents the summary.
- User can then invoke `/execute-manual-tests` to run it.
