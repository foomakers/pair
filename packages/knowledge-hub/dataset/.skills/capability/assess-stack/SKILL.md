...
3. **Act** (implementation/review mode): Add or update **only the affected entry** in the appropriate section. Preserve all other content, including sections owned by /assess-testing and /assess-ai.
...
- Version updates go through /assess-stack: check compatibility with existing stack, update adoption, write decision record.
...
- `/bootstrap` includes adoption and ADL changes in the next commit.
...
- If rejected (incompatible) → `/implement` HALTs.
...
- If rejected → `/review` includes as CHANGES-REQUESTED finding.
...
- If tech-stack.md doesn't exist (bootstrap mode), create it. If implementation/review mode, HALT: "No tech stack defined — run /assess-stack in bootstrap mode first."
