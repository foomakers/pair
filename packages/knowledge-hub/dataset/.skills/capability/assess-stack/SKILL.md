...
- **Persistence**: `/bootstrap` composes `/record-decision`, then includes those changes in the next commit.
...
When invoked **independently**: mode auto-detected. The skill returns the proposal; the human (or agent) persists it by composing `/record-decision`, then commits.
...
- If tech-stack.md doesn't exist in implementation/review mode (as opposed to bootstrap, covered above), HALT: "No tech stack defined — run /assess-stack in bootstrap mode first."
