...
6. Publish the handoff (`skill: "review-phase"`, `inputHead: $head or $base`, `mode`, `tier`, `passes`, `reviewedHead`, `verdict`, `findings`, `custody`, `readiness`, `attempt: $attempt`, `reviewer`, `partial`, `published: { firstReview, synthesis, escalation }`, `gates: { command, identity, exitCode }`, `inputsDigest: $inputs`, `acHash` (the `cycle-state.mjs ac-hash --story $story` value — never a summary), `reviewer`, `partial`, `elapsedMs`) with `cycle-state.mjs publish … --attempt $attempt --predecessor <a0-implement-phase | r<n>-g<k>-green-fix | r<n-1>-review-phase> --pr $pr`, run `resolve` again and return its `next`.
