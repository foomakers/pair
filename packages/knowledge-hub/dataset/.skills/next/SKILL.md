...
Analyze project state and recommend the single most relevant next skill to invoke. Covers the full 51-skill catalog across all lifecycle phases.
...
The catalog is **derived from the installed corpus**: every skill directory under `.skills/` must appear here — 10 process + 32 capability + 7 workflow + `/next` and `/loop` (the two bare, uncategorized navigator skills) = 51. If an installed skill is missing from these tables (or a row names a skill that is not installed), the catalog has drifted: update the tables, the stated counts, and the cascade rows together.

### Process Skills (10)

| Skill              | Lifecycle Phase    | Description                                     |
| ------------------ | ------------------ | ----------------------------------------------- |
| `/brainstorm`      | Discovery          | Structured discovery: interview, domain, tree   |
...
| `/analyze-delivery-metrics` | Analysis     | Delivery/AI metrics for a period (bug resolution, PR lead time, adoption) → one period panel |
...
Delivery-stage skills under `workflow/`, dispatched by the `pair-implement-batch` engine or by the in-session coordinator `/cycle` — the four judgment stages of a delivery cycle, the batch-level template contract and the coordinator itself. They are catalogued for completeness and never cascade-suggested: a human does not invoke a remediation phase from a board state.

| Skill | Stage | Description |
| ----- | ----- | ----------- |
| `/red-spec` | 1 | Preparation: inventory, grouping, executable acceptance contract (witnesses + controls), test-only |
| `/red-verify` | 2 | Independent validation of the contract, all gaps in one answer, then the deterministic seal |
| `/implement-phase` | 3 | Initial implementation above the sealed contract, test-first; publishes the story's one PR |
| `/green-fix` | 3 | Remediation implementation inside fixScope above the seal; updates the PR |
| `/review-phase` | 4 | Independent final verification: custody, evidence, tier passes, stable findings, idempotent publication |
| `/contract-phase` | 0 | Ensure the template-derived machine contract (cache-by-hash via ensure-contract.mjs) |
| `/cycle` | — | In-session coordinator: drives one card through the whole cycle (or fix & review on a PR) from a Claude Code / Codex session, one stage per subagent |

### Navigator Skills (2)

The two bare, uncategorized entry points — every other skill lives under `process/`, `capability/` or `workflow/`.

| Skill    | Description                                                                              |
| -------- | ----------------------------------------------------------------------------------------- |
| `/next`  | This skill — recommends the single most relevant next skill from project/PM-tool state    |
| `/loop`  | Unattended delivery loop over an automation policy (#250) — fan-out in Claude Code, a degraded one-card + continue-token path elsewhere |
...
- **Epic root** → rows 3–4 are skipped (an epic contains no initiatives or epics); **row 5 is kept, scoped to the root epic** — if it has no user-story children, select `/plan-stories`. This is the epic-root "plan its children" action.
