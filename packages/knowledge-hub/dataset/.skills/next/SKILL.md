...
Analyze project state and recommend the single most relevant next skill to invoke. Covers the full 52-skill catalog across all lifecycle phases.
...
The catalog is **derived from the installed corpus**: every skill directory under `.skills/` must appear here — 10 process + 32 capability + 8 workflow + `/next` and `/loop` (the two bare, uncategorized navigator skills) = 52. If an installed skill is missing from these tables (or a row names a skill that is not installed), the catalog has drifted: update the tables, the stated counts, and the cascade rows together.
