...
The resolved candidate set feeds the scoped Step 3 item-selection (rows 6–11) and Step 4 row 16 (`/grill`) — not the project-wide detectors (Step 2, rows 3–5, rows 12–15). Because Step 0 re-runs each time, a tag mutation between steps changes the selection on the next step automatically.
