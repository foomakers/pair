...
| `/classify`    | Capability | Optional — shift-left classification matrix into the story body. If not installed, warn and continue. |
...
4. **Verify**: The story body carries the classification matrix (or the skip was warned). `/classify` HALTs only if the quality model doc (#221) is absent — surface that pointer to the developer.
