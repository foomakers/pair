...
| `/map-subdomains` | Capability | Optional — phase 2 placement for a broad/functional discovery, scoped. If not installed, warn and skip domain placement.                                            |
| `/map-contexts`   | Capability | Optional — phase 2 placement for a punctual/technical discovery, scoped. If not installed, warn and skip context mapping.                                           |
| `/plan-epics`     | Process    | Conditional — phase 3 writer for a broad tree (epics, then their stories). If not installed, present the confirmed tree for manual entry.                           |
| `/plan-stories`   | Process    | Conditional — phase 3 writer for a punctual tree (stories under the root). If not installed, present the confirmed tree for manual entry.                           |
...
| ------------------------------------------------ | ---------- | ----------- | ---------------------------------------------------- |
| type `epic` (or `initiative`)                     | broad      | functional  | `/map-subdomains`                                    |
| type `story`                                      | punctual   | functional  | `/map-contexts`                                      |
| any type + a `tech` / `tech-debt` / `infra` tag    | unchanged  | technical   | `/map-contexts`                                      |
| type `epic` + a technical tag                     | broad      | technical   | `/map-subdomains`, then `/map-contexts` (same scope)  |
...
4. **Act**: Apply the [Orientation Matrix](#orientation-matrix) to the loaded issue's type and tags, then state the deduction up-front as a proposal:
   ...
   > Phase 2 will run [`/map-subdomains` | `/map-contexts`]. Proceed, or override level/orientation?
...
2. **Act**: Compose the placement capability the [Orientation Matrix](#orientation-matrix) selects, always **scoped to the areas the blob touches, never `$scope: all`** (that is `/bootstrap`-only):
...
2. **Act**: No PM tool → **HALT** here with the setup pointer (`/setup-pm`); phases 1–2 keep their output (blob handoff + updated domain files), so re-running after setup starts at this phase.
...
5. **Act**: Triage the candidate tree against the **existing** backlog under that parent scope, per the shared [to-issues-triage.md](../../../.pair/knowledge/guidelines/technical-standards/ai-development/skill-conventions/to-issues-triage.md) convention — extend-vs-create is decided against the existing tree, so discovery merges into the backlog instead of duplicating it. Compose the writer the level selects:
   - Broad → `/plan-epics` with the candidate tree (then `/plan-stories` for the slices under each confirmed epic).
   - Punctual → `/plan-stories` with the candidate tree.
...
6. **Act**: Items land as **Draft**: discovery produces planning units, and `/refine-story` is what takes a story from Draft to Ready. Report a discovery that changes the product vision as a recommendation to run `/specify-prd` — the PRD is never modified here.
