...
| `/plan-stories`   | Process    | Conditional — phase 3 writer when the root is an **epic** (stories under it) or a **story** (sibling stories under the root's parent epic). If not installed, present the confirmed tree for manual entry. |
...
† **Required _when installed_.** `/grill` carries Required = Yes because phase 1 composes it **by default** — but it **degrades gracefully**: brainstorm warns and runs the interview inline (Phase 1 item 4, and under Graceful Degradation) when grill is absent, never HALTing on it. "Required" here means _composed by default_, not _a hard prerequisite_, so the flag never contradicts the degrade-inline step.
...
Deduction inputs, in precedence order: **argument > tags > issue type**. The type decides the level **and** the phase-3 writer; tags decide the orientation and win over the type-derived default. **Most specific row wins**, and the tag row is a **modifier**, not a competing row: it flips the orientation and adds `/map-contexts`, leaving the level and the writer as the type row resolved them.
...
| any type + a `tech` / `tech-debt` / `infra` tag (modifier) | unchanged  | technical   | `/map-contexts` — **in addition to, and after**, the type row's capability when that row is broad | unchanged (the type row's)                                                                                                     |
| no `$root` (free theme)                                | asked first (phase 1) | follows the answered level, `functional` default | resolved once the level is answered                    | broad → `/plan-epics` with `$initiative: <the initiative the placement points to, confirmed>`; punctual → `/plan-stories` with `$epic: <the epic the placement points to, confirmed>` |
...
4. **Act**: Apply the [Orientation Matrix](#orientation-matrix) to the loaded issue's type and tags, then state the deduction up-front as a proposal:
   ...
   > Phase 2 will run [`/map-subdomains` | `/map-contexts` | both]; phase 3 will write with [`/plan-epics` | `/plan-stories`] under `#[parent ID]` ([the root | the root's parent epic | the initiative the placement points to]).
...
4. **Act**: Compose `/grill` with `$mode` omitted (interview), `$topic`: the free theme (`$theme`) or the root issue's subject, and `$context`: the loaded `$root` issue plus the resolved level/orientation, so grill explores before asking instead of re-fetching. Grill returns the **raw requirements blob** (R3.7) — findings per sub-question, open items, flagged assumptions.
5. **Act**: If the session stops before the interview reaches explicit shared understanding, take grill's handoff offer: the blob-so-far is written to `.pair/working/` so a later `/brainstorm` run resumes from it (Step 2 above) rather than restarting.
...
7. **Act**: Triage the candidate tree against the **existing** backlog under that parent scope, per the shared [to-issues-triage.md](../../../.pair/knowledge/guidelines/technical-standards/ai-development/skill-conventions/to-issues-triage.md) convention — extend-vs-create is decided against the existing tree, so discovery merges into the backlog instead of duplicating it. Compose the writer resolved in item 6, passing **both** the parent and the tree:
   - `/plan-epics` with `$initiative: [the resolved initiative]` and `$candidates: [the candidate tree]` — then, for the slices under each confirmed epic, `/plan-stories` with `$epic: [that epic]` and `$candidates: [its slices]`.
...
- **No parent to hang the tree from** (Phase 3, free-theme discovery only) — no initiative exists for a broad tree, or no epic for a punctual one; HALT with the `/plan-initiatives` / `/plan-epics` pointer rather than writing beneath an unrelated parent.
