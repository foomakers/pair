...
- **Caller-supplied tree** — with `$candidates` (e.g. `/brainstorm` phase 3), Step 3 triages the supplied candidates and derives none: the caller owns the grouping, this skill owns triage, the epic template, and the writes. Without it, behaviour is unchanged.
