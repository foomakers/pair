...
4. **Verify**: Record quality gate results. A run reported with `Tree: ⚠️` (the checked-out tree is not the PR's head) is **advisory only**: record it as a finding aid, do **not** feed it to the Step 5.4 `resolve_pr_state <gates> …` synthesis — for a mismatched tree the authoritative gate signal is CI's check on the PR **head commit**, so read the gates from there (a green local run over unrelated code can never contribute to `ready-to-merge`). **Name the read, don't improvise it**: read the head commit's checks from the **code host** — `gh pr checks "$pr"` for GitHub, substituting the host's equivalent per the [routing table](../../../.pair/knowledge/guidelines/technical-standards/ai-development/skill-conventions/way-of-working-pm-resolution.md) — and treat any conclusion other than `success` on a **required** check as **not green** (the required set is the branch protection's, see [github-implementation.md](../../../.pair/knowledge/guidelines/collaboration/project-management-tool/github-implementation.md) § PR state flow). **CI has published no conclusion on that head commit** — the normal state in the first minutes after a push, and the permanent state on a host that runs no checks — is **not** a green: pass `gates` as **not green** (`Gates: pending`), so Step 5.4 yields `pr-state:to-be-reviewed` and **never** `ready-to-merge`; the remedy is re-reading the check once CI reports, never a promotion on absence of evidence. **Which signal caps the verdict** follows the same split: the cap keys on the **authoritative** signal — the local run when the tree matches the PR head, CI's conclusion on the head commit when it does not. An advisory (tree-mismatched) run therefore contributes review **findings only**; it never caps the verdict on its own, since the PR would otherwise be blocked on evidence this same step just declared advisory. With the **authoritative** gates red the review can **never** reach APPROVED / TECH-DEBT and the Step 5.4 synthesis can **never** yield `ready-to-merge` — the judgment review does not override a mechanical failure. Continue the review (the findings are still useful to the author) but carry `Gates: red` into Step 5.2. A raise landing **after** this step (Step 2.4 / 2.6 reconcile Security relevance and Coupling balance, raise-only per D17) does not retro-narrow anything: the set that ran is Phase 1's tier, and the remedy is a **re-run** of this step at the higher tier, never a report rewritten to a narrower one.
