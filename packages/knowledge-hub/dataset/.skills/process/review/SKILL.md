...
4. **Verify**: The review matrix + `risk:*` tier are recorded on the PR (matrix in the body as 1 line + `<details>`, D22; tags applied only when a `## Tag Projection` is declared). A raise to `risk:red` is carried into the Step 5.2 decision. `/classify` HALTs only if the quality model doc (#221) is absent.
...
4. **Verify**: Verdict + findings recorded — feeds the Security Review section (Step 5.1) and the **Security relevance** dimension of the Step 1.5 classification matrix (`/classify` folds this verdict in **raise-only** — it may raise the tier, never lower it). If any **introduced** finding is red → flag explicitly: this is the AC4 signal that drives the CHANGES-REQUESTED decision in Step 5.2. Does not itself HALT — `/assess-security` has no merge authority, this skill's own decision step does.
...
See [idempotency convention](../../../.pair/knowledge/skill-conventions/idempotency.md). Re-invoking `/review` on a partially reviewed PR is safe — per-phase:
