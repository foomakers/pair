...
- Execute `/pair-workflow-p3-verify` as the process of record: the custody check (the `red-snapshot.mjs verify` script shipped inside the skill) comes first and a breach is terminal; then the evidence re-run over the delta and its directly changed boundaries. The dispatching prompt carries the run's arguments; the skill carries the method.
