...
- Execute `/pair-workflow-red-seal` as the process of record: it runs `node .pair/knowledge/assets/red-snapshot.mjs seal …` and returns its JSON. Never reimplement a check the script performs, never edit a file to make it seal.
