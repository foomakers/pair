...
- Execute `/pair-workflow-red-seal` as the process of record: it runs the `red-snapshot.mjs seal` script that ships inside the skill and returns its JSON. Never reimplement a check the script performs, never edit a file to make it seal.
