...
| --- | --- | --- | --- | --- |
| **Bug resolution time** | How long does a defect stay open? | bug issue created | bug issue closed | hours (days when the median exceeds 48h) |
...
| **Classification coverage** | items carrying a classification matrix or a projected `risk:*`/`cost:*` tag ÷ items delivered | PM tool item bodies/labels |
| **Review coverage** | merged PRs whose `pair-review` check reported ÷ merged PRs | code host checks/statuses |
...
| Review-check presence | check runs / statuses on the merge head | pipeline/policy evaluation on the PR | from the code host | n/a |
