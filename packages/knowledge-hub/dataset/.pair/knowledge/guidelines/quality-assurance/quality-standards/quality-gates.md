...
export class PreDevelopmentQualityGates {
  ...
  private generateGateDetails(gate: PreDevelopmentGate, results: CriterionResult[], overallScore: number): string {
    const passedCount = results.filter(r => r.passed).length;
    const totalCount = results.length;
    
    return `Gate ${gate.name}: ${passedCount}/${totalCount} criteria passed, overall score: ${overallScore.toFixed(1)}%`;
  }
  ...
  public async executeCodeQualityGate(codebase: CodebaseContext): Promise<QualityGateResult> {
    ...
    const criteria = [
      ...
      },
      {
        ...
        passed: securityResult.criticalVulnerabilities === 0 && securityResult.highVulnerabilities === 0,
    ...
    const overallScore = criteria.reduce((sum, criterion) => sum + criterion.score, 0) / criteria.length;
    const allCriticalPassed = criteria.filter(c => c.name === 'Security Vulnerabilities' || c.name === 'Test Coverage').every(c => c.passed);
    ...
    return {
      ...
      details: `Code Quality Gate: ${criteria.filter(c => c.passed).length}/${criteria.length} criteria passed`
    };
  }
      ...
      if (!criterion.passed) {
        switch (criterion.name) {
          ...
          case 'Code Duplication':
            ...
            break;
          case 'Test Coverage':
            recommendations.push('Increase test coverage by adding missing unit and integration tests');
            break;
...
export class PostDevelopmentQualityGates {
  ...
  public async executeIntegrationGate(environment: string): Promise<QualityGateResult> {
    ...
    return {
      ...
      details: `Integration Gate: ${criteria.filter(c => c.passed).length}/${criteria.length} criteria passed`
    };
  }
  ...
  public async executeE2EGate(environment: string): Promise<QualityGateResult> {
    ...
    const overallScore = criteria.reduce((sum, c) => sum + c.score, 0) / criteria.length;
    const criticalPassed = criteria.filter(c => c.name.includes('Critical') || c.name.includes('User Journey')).every(c => c.passed);
    ...
    return {
      ...
      details: `E2E Gate: ${criteria.filter(c => c.passed).length}/${criteria.length} criteria passed`
    };
  }
...
export class QualityGateCoordinator {
  ...
  public async executeQualityPipeline(context: QualityPipelineContext): Promise<QualityPipelineResult> {
    const startTime = Date.now();
    ...
    try {
      ...
      const pipelineResult = this.createPipelineResult(context, results, pipelinePassed, Date.now() - startTime);
      ...
      return this.createPipelineResult(context, results, false, Date.now() - startTime, error.message);
    }
  }

  private async executePreDevelopmentGates(context: QualityPipelineContext): Promise<QualityGateResult[]> {
    const results: QualityGateResult[] = [];
    
    if (context.gates.includes('requirements')) {
      results.push(await this.preDevGates.executeGate('requirements-gate', context.requirementsContext));
    }
  ...
  private async executeDevelopmentGates(context: QualityPipelineContext): Promise<QualityGateResult[]> {
    const results: QualityGateResult[] = [];
  ...
  private async executePostDevelopmentGates(context: QualityPipelineContext): Promise<QualityGateResult[]> {
    const results: QualityGateResult[] = [];
  ...
  private generatePipelineRecommendations(results: QualityGateResult[]): string[] {
    ...
    if (failedGates.length > 0) {
      recommendations.unshift(`Address ${failedGates.length} failed quality gates before proceeding`);
    }
