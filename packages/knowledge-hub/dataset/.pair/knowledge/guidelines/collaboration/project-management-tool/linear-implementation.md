...
2. **Item → PR**: the PR URL is posted back as a **comment on the Linear issue** right after the PR exists, completing the bidirectional link. `/publish-pr` does this through `/write-issue $mode: comment` — a comment, never an issue-body update, so the story's description is untouched; the underlying mutation is:
