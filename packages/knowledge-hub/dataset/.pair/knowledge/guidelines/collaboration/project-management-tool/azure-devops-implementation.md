...
The **work-item** comment mechanism (distinct from the PR threads under Code Review below) — what `/write-issue $mode: comment` resolves for `azure-devops`. `az boards` exposes no comment verb, so it goes through the work-item comments REST endpoint; nothing else on the item is written (no field, no state).
