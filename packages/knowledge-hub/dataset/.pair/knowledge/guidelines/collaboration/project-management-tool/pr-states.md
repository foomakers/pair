...
| --- | --- |
| `/publish-pr` | Creates the PR **without classification tags** (the review classifies it; untagged = fail-safe red until then), registers `pair-review` as **pending**, labels the PR `pr-state:to-be-reviewed`, and triggers the review in a clean-context subagent |
