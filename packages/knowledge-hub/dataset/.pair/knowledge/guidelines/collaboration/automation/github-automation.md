...
An issue carrying no mapped tag **runs nothing by tag**: no mapped workflow is dispatched, so a skip prints no `DISPATCH-RECORD:` line and the comment step posts nothing. That includes the **unlabelled** issue, where `join(github.event.issue.labels.*.name, ',')` renders an empty string: `--card-tags` reads an empty value as the observation "this card carries no labels", so the adapter needs no guard clause and no conditional call — passing what the trigger saw is always correct. The same holds when `tech/automation.md` declares no `## Workflows` section at all — the run reports `no mapping declared`.

What `pair-cli run` does next is the [card-readiness fallback](automation-policy.md#pair-cli-run---card-on-an-unmapped-card-the-cards-own-readiness-decides), and with this adapter's `--autonomous` it is bounded: where `## Eligibility` is declared, a card without its label is skipped `ineligible` (with none declared there is no label to require — the explicit-`--card` exception in [automation-policy.md](automation-policy.md#absent-file-or-absent-section--empty-eligibility-set)); a `Draft` card, or a `Ready` one without a task breakdown, is skipped as "needs a human" (a preparation skill never runs unattended); an eligible `Ready` card with a breakdown **enters the delivery cycle** — a dispatch like any other: it takes the card's lock, writes `start`/`end` to the audit trail and prints the `DISPATCH-RECORD:` line (`workflow=pair-workflow-cycle`), which the comment step posts. A runner whose `gh` cannot read the card (not provisioned, not authenticated) reports `card-unreadable` and exits `0` when no `## Workflows` is declared. The boundary lives in the routing core precisely so that no adapter can widen it by accident — and a project that wants only tagged cards to run declares `## Eligibility`.
