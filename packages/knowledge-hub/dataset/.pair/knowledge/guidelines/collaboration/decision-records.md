...
- Rationale: architectural and domain decisions form supersede chains and are referenced by number elsewhere (brainstorm/refine conflict flags, cross-references) — a chain reads more naturally as `adr-003 supersedes adr-001` than as date deltas. See the `2026-07-12-adr-sequential-naming-convention.md` ADL (updated 2026-07-18 with the DDR/ADL/Analysis-Log rationale) — not linked here since this dataset excerpt's `decision-log/` only mirrors one example entry, not the full adoption log.
