...
- **Partial coverage** (the run could not process every item of the period): the panel is still written, with the shortfall **stated in the headline** (`Coverage: N of M ... processed` + reason + the items left out). A large period is never a reason to refuse a panel — process the period in batches, retaining per item only what the panel renders — but a partial panel must never look complete.
