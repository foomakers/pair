...
One order, stated once — **the argument first, then the adoption default, then none** — and both writers apply it unchanged (`/write-issue` for the item, `/publish-pr` for the PR). A cascade defined twice is a cascade that drifts, and the drift is silent: the failure mode is an invisible item, not an error.
