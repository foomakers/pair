...
A **process profile** declares which of the [catalogued process steps](step-catalogue.md) a project runs. A team adopting a subset of the process configures it once, in adoption, and no disabled step is ever proposed — by `/next`, by a skill, or by the how-to path a project with **no skills installed** follows.
...
**A key written under no such section at all HALTs too, named by its line.** Every rule above is about the heading's SHAPE, so each one closes the spellings it was written for and a key under `## Process Profiles` (the plural — this page's own title), `## ProcessProfile`, `## Setup` or no heading whatever would be read by nothing and reported by nothing: `default`, the whole process re-enabled, byte-identical to writing nothing. The key is what is reported, so every one of those cases HALTs at once. A key-shaped line inside a code block, an HTML comment or a documentation table row stays what it was — an example, a comment, documentation.

Inside the section, **all three CommonMark code-block forms are examples, never declarations** — ```` ``` ````-fenced, `~~~`-fenced and four-space-indented. `~~~` is the standard way to nest a block that itself contains backticks, and a team documenting its own choice next to an alternative shape would otherwise either lose steps silently (the indented example silently BECOMING the profile) or be unable to run `/next` at all, on an error naming a placeholder id it never configured. The indented form is the one ambiguous with real configuration, hence the HALT above: as an example it is skipped only when it carries no key line.
...
| ------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- |
| `/next`                               | Never proposes a disabled step; reports the HALTs above and the prerequisite inconsistency.                             |
...
Selecting a profile through a `/next` argument is a **future extension**. Today the profile is a property of the project, declared once in adoption, not a per-invocation switch.
