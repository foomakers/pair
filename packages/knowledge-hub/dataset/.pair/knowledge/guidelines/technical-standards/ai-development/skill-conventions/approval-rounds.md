...
| **Judgement gate** — the round has **no proposal to accept**, because the missing input is a human judgement the skill cannot supply (a risk to mitigate or accept, a conflict with no safe default) | **It still HALTs.** `auto` suppresses *asking*, never *judging*. A gate is not an approval round with the human removed; it is the case where proceeding would record something nobody judged. |

The line between row 1 and row 3 is the whole point: **can the skill state, on its own, what the human would be accepting?** If yes, `auto` accepts it. If the answer is "whichever of these the human prefers", there is nothing to auto-accept and the gate fires.
