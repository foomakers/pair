...
export class BufferManager {
  ...
  private cleanup(): void {
    ...
    for (const [size, pool] of this.buffers) {
      while (pool.length > 10) { // Keep max 10 buffers per size
...
- [Caching Patterns](performance-patterns-caching.md) - Memory-efficient caching
- [Database Optimization](performance-patterns-database.md) - Memory usage in data access
- [Concurrency Patterns](performance-patterns-concurrency.md) - Memory safety in concurrent code
- [Auto-Scaling](scaling-patterns-auto-scaling.md) - Memory-based scaling decisions
