...
export class CodeReviewOrchestrator {
      ...
      if (session.request.storyId && session.request.scope.includeStoryValidation) {
        ...
        if (storyValidation.acceptanceCriteriaIssues.length > 0) {
          issues.push({
            ...
            files: storyValidation.affectedFiles,
            line: null,
            suggestions: storyValidation.acceptanceCriteriaIssues.map(
              issue => `Fix acceptance criteria: ${issue.criterion}`,
            ),
          })
          sectionScore -= 15
        }
        ...
        for (const taskId of session.request.taskIds) {
          ...
          if (taskValidation.qualityIssues.length > 0) {
            issues.push({
              ...
              files: taskValidation.affectedFiles,
              line: null,
              suggestions: taskValidation.qualityIssues.map(
                issue => `Address quality issue: ${issue.description}`,
              ),
            })
            sectionScore -= 10
          }
  ...
  private async executeCodeQualityReview(session: ReviewSession): Promise<ReviewSectionResult> {
    ...
    const maintainabilityAnalysis = await this.qualityAnalyzer.analyzeMaintainability(
      session.changedFiles,
    )
    ...
    return {
      ...
      duration: Date.now() - startTime,
      issues,
      summary: this.generateCodeQualitySummary(
        complexityAnalysis,
  ...
  private generateRecommendations(sections: ReviewSectionResult[]): string[] {
    ...
    const hasSecurityIssues = sections.some(
      s => s.section === 'security-review' && s.status !== 'passed',
    )
    const hasQualityIssues = sections.some(
      s => s.section === 'code-quality' && s.status !== 'passed',
    )
...
export class ReviewCompletenessValidator {
  ...
  public async validateReviewCompleteness(
...
export class ReviewQualityMetrics {
  public static calculateReviewThoroughness(review: CodeReviewResult): number {
    ...
    const sectionCoverageScore =
      (sections.length / this.getExpectedSectionCount(review.request.scope)) * 100
