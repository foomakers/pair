...
export function setupPerformanceMonitoring() {
  const performanceMonitor = new PerformanceMonitor(logger, metricsCollector, {
...
export function usePerformanceMonitoring(componentName: string, props?: any) {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      performanceMonitor.markCustomMetric(`component_mount_time_${componentName}`, mountTime, 'ms')
    }
  ...
  useEffect(() => {
    ...
    performanceMonitor.markCustomMetric(`component_render_time_${componentName}`, renderTime, 'ms')
...
export function createLazyComponent<T extends ComponentType<any>>(
  ...
  const { retryCount = 3, timeout = 10000, preload = false } = options
  ...
  const loadComponent = async (): Promise<{ default: T }> => {
    ...
    try {
      ...
      performanceMonitor.markCustomMetric('component_lazy_load_time', loadTime, 'ms')

      return module
    } catch (error) {
      importPromise = null // Reset for retry

      performanceMonitor.markCustomMetric('component_lazy_load_error', 1, 'count')
...
export function PerformanceSuspense({
  ...
  useEffect(() => {
    ...
    performanceMonitor.markCustomMetric(`suspense_time_${suspenseId}`, suspenseTime, 'ms')
  })
...
export class MemoryMonitor {
  ...
  private checkMemoryUsage(): void {
    ...
    if (used > this.thresholds.critical) {
      console.error(
        `Critical memory usage: ${this.formatBytes(used)} (${usagePercentage.toFixed(1)}%)`,
      )
...
export function useMemoryLeakDetection(componentName: string) {
  ...
  useEffect(() => {
    return () => {
      ...
      const totalCleanups =
        subscriptionsRef.current.size + timersRef.current.size + intervalsRef.current.size
