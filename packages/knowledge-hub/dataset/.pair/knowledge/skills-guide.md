...
4. **Pruning** — single source of truth per meaning (shared conventions live once; skills keep deltas); every line passes a relevance test; every sentence passes a no-op test (strip it out — would the model act differently without it? delete whole failing sentences); prune on every edit before sediment settles. *Check: the meaning you're adding doesn't already live elsewhere.*
