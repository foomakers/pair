...
|-------|-------|-------------|
| `/remediation-plan` | D0 | Group actionable findings by owner, mode and allowed paths into a frozen plan |
| `/red-spec` | D1 | Author the test-only RED contract for one group (domain map, fixScope, hashed artifacts) |
| `/red-verify` | D2 | Independently reproduce the RED contract before it is sealed |
| `/red-seal` | D3 | Freeze the contract into one local Git snapshot via `red-snapshot.mjs seal` |
| `/green-fix` | D4 | Make the sealed contract pass inside fixScope; commit GREEN above the seal; update the PR |
| `/p3-verify` | D5 | Custody check via `red-snapshot.mjs verify`, then read-only evidence re-run over the delta |
| `/review-phase` | C | Independent blind review of one PR head: review set by risk tier, union of passes, first / re-review / fresh modes |
| `/cycle-comments` | C/D | The cycle's PR-comment policy: probe (marker + log), flush (escalation), synthesize (one final remediation table) |
