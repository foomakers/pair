...
See `/bootstrap` Phase 0 and Phase 1 for operational details.
...
See `/bootstrap` Phase 2 for section-specific questions and assessment flow.
...
1. Offer the `## Criticality Table` and `## Overrides` sections of `tech/risk-matrix.md` — the two classification-delta sections nothing else authors (`/classify` self-proposes `## Tag Projection` only).
2. Confirmed sections are written with the same propose-then-write-if-confirmed registry pattern `/classify` uses for its own section — registry state, so not via `/record-decision`.
...
See `/bootstrap` Phase 3.6 for the question set and the write path.

### Phase 3: Finalization

1. Verify consistency across all standards documents
2. Configure PM tool via `/setup-pm`
3. Establish update process for future iterations
4. Get developer final approval

See `/bootstrap` Phase 4 for verification and PM configuration flow.
