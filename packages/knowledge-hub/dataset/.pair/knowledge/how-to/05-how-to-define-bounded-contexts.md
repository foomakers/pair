...
**Skill**: When `/map-contexts` is available, invoke it — it automates the operational steps of this workflow (constraint extraction, context catalog, integration validation, file creation). This how-to describes the workflow and its HALT conditions.

## Skill Composition

This how-to orchestrates the `/map-contexts` skill.

| Skill            | Purpose                                                                                                   |
| ---------------- | --------------------------------------------------------------------------------------------------------- |
| `/map-contexts`  | Executes the full process: subdomain synthesis, context boundary analysis, BC file creation.               |
...
2. **Invoke `/map-contexts`** with optional `$scope` argument (`all` or `single`). The skill handles:
