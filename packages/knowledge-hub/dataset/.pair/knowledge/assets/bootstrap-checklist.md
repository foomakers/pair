...
- **Service/domain criticality** — the `## Criticality Table` / `## Overrides` delta of `tech/risk-matrix.md`, offered in bootstrap's Phase 3.6. Which services are High is a property of the project, not of its size, and a guessed map would silently shape every future risk tier. A quick run writes **no delta**: the file stays absent and the KB defaults apply ([quality model](../guidelines/quality-assurance/quality-model.md) §3.1/§6).
