...
Optional threshold tweaks for the dimensions of §3.1 of the quality model. Omit entirely if the KB defaults are fine — every key here is opt-in.
...
- `business-impact.trivial-diff: green` — an objectively trivial change (every changed file is `.md`/`.mdx`, **or** every changed hunk is comment-only, whitespace-only or formatter-output-only) resolves Business impact to `green` whatever subdomain it touches, so a doc typo fix in `checkout` is not tiered as if it changed checkout. All-or-nothing: one non-trivial file or hunk and the dimension falls back to the subdomain class. Full definition and edge cases in [quality-model.md §6](../guidelines/quality-assurance/quality-model.md).
