...
describe('regenerate-mirrors.sh — the local, deterministic mirror remedy (#419)', () => {
  ...
  it(
    ...
    () => {
      ...
      const MSG = '[#419] chore: regenerate mirrors from local dataset'
      git(tmp, ['add', REL])
  ...
  it(
    'leaves an untracked file under an overwrite or skip tree byte-identical and unindexed — the HALT scope is the UNSAFE trees, not the written ones',
    () => {
      // Round-7 finding (Minor), executed. Round 6 scoped the pre-run HALT to "the trees the
      // command writes into", but the harm it guards exists only where the registry BEHAVIOUR
      // destroys or indexes a foreign file — and the command also writes into trees whose
      // behaviour does neither. CONCRETE FAILURE of the round-6 scope: untracked
      // `.claude/settings.local.json` (Claude Code writes it on the first permission grant; this
      // repo's `.gitignore` does not list it) and an untracked `.github/workflows/wip.yml` both sit
      // under the written trees, so /publish-pr HALTed with a stash remedy on EVERY publish — for
      // files this run provably leaves byte-identical, entry-identical and unindexed.
      //
      // The complete table, one row per registry behaviour the producer declares
      // (apps/pair-cli/config.json; index sections: apps/pair-cli/src/registry/llms-generation.ts):
      //   DELETED   `.pair/knowledge/**`            knowledge  mirror
      //   DELETED   `.github/agents/**`             github     mirror, `include: ["/agents"]`
      //   INDEXED   `.pair/adoption/{product,tech,decision-log}/**/*.md`  adoption  add + llms
      //   OVERWRITTEN (digest row, not this check)  `AGENTS.md`  agents  mirror over ONE file
      //   UNTOUCHED `.github/workflows/**`, `.github/ISSUE_TEMPLATE/**`   github  skip
      //   UNTOUCHED `.github/skills/**`  — a symlink INTO `.claude/skills`, so the file lands there
      //   UNTOUCHED `.claude/**`  skills / workflows / agent-definitions  overwrite; settings.local.json
      //   UNTOUCHED `.pair/adoption/` outside the three indexed sections; a non-.md inside them
      //   UNTOUCHED `.pair/<anything else>`
      // Every row is asserted here so the adoption's unsafe list is MEASURED against the real
      // script, not derived from the config by reading. A row that flips (a section added to the
      // index, a folder added to `include`) fails this test before it can fail a contributor.
      tmp = makeFixture()
      const dataset = join(tmp, 'packages/knowledge-hub/dataset')
      // The real dataset ships `.github/agents/`; the fixture needs the folder for the `include`
      // row to be exercised as the CLI actually runs it.
      write(join(dataset, '.github/agents/stub.agent.md'), '# stub agent\n')
      // The finding's point about `.claude/settings.local.json` is that only a maintainer's
      // GLOBAL ignore hides it. That global ignore applies to this fixture too, and would hide
      // the row from the test on exactly the machines where the HALT never fired.
      git(tmp, ['config', 'core.excludesFile', '/dev/null'])
      write(join(tmp, '.pair/knowledge/index.md'), '# pre-existing install\n')
      expect(run(tmp, isolatedHome(tmp)).status).toBe(0)
      git(tmp, ['add', '-A'])
      git(tmp, ['commit', '-q', '-m', 'converged'])
      // An untracked AGENTS.md is impossible in a repo that tracks it; the fixture untracks it to
      // measure what a one-file mirror registry does to a foreign file (overwrite, not delete).
      git(tmp, ['rm', '-q', '--cached', 'AGENTS.md'])
      git(tmp, ['commit', '-q', '-m', 'untrack AGENTS.md'])

      const DELETED = ['.pair/knowledge/how-to/wip.md', '.github/agents/wip.agent.md']
      const INDEXED = '.pair/adoption/tech/wip-note.md'
      const OVERWRITTEN = 'AGENTS.md'
      const UNTOUCHED = [
        '.github/workflows/wip.yml',
        '.github/ISSUE_TEMPLATE/wip.md',
        '.claude/settings.local.json',
        '.claude/skills/wip.json',
        '.claude/workflows/wip.js',
        '.pair/adoption/wip-root.md',
        '.pair/adoption/notes/wip.md',
        '.pair/adoption/tech/wip.txt',
        '.pair/wip.md',
      ]
      // `.github/skills` is a symlink the install created; a file written "under" it lands in
      // `.claude/skills` — the porcelain names the resolved path, so it is asserted there.
      const VIA_SYMLINK = { written: '.github/skills/wip.md', resolved: '.claude/skills/wip.md' }
      for (const rel of [...DELETED, INDEXED, ...UNTOUCHED]) write(join(tmp, rel), `# ${rel}\n`)
      ...
      const beforeEntries = parsePorcelainZ(before.entries)
      expect(beforeEntries.map(entry => entry.path).sort()).toEqual(
        [...DELETED, INDEXED, OVERWRITTEN, ...UNTOUCHED, VIA_SYMLINK.resolved].sort(),
      )
      expect(beforeEntries.every(entry => entry.xy === '??')).toBe(true)
      expect(readFileSync(join(tmp, LLMS), 'utf-8')).not.toContain('wip')
      ...
      const llms = readFileSync(join(tmp, LLMS), 'utf-8')

      // DELETED rows: gone from disk and from the porcelain — and NOT indexed, because the mirror
      // cleanup runs before the index is generated (the `how-to` row is under an llms section).
      for (const rel of DELETED) {
        expect(existsSync(join(tmp, rel))).toBe(false)
        expect(afterEntries.find(entry => entry.path === rel)).toBeUndefined()
        expect(llms).not.toContain(rel)
      }
      // INDEXED row: survives byte-identical, entry unchanged — and the index now links it.
      expect(git(tmp, ['hash-object', INDEXED]).trim()).toBe(before.digests.get(INDEXED))
      expect(afterEntries).toContainEqual({ xy: '??', path: INDEXED })
      expect(llms).toContain(`(${INDEXED})`)
      expect(afterEntries).toContainEqual({ xy: ' M', path: LLMS })
      ...
      // the round-6 scope HALTed on; a HALT here blocks a routine state for a loss that does not
      // happen.
      for (const rel of [...UNTOUCHED, VIA_SYMLINK.resolved]) {
        expect(existsSync(join(tmp, rel))).toBe(true)
        expect(git(tmp, ['hash-object', rel]).trim()).toBe(before.digests.get(rel))
        expect(afterEntries).toContainEqual({ xy: '??', path: rel })
        expect(llms).not.toContain(rel)
      }
      // The whole after-porcelain is exactly: every surviving `??` plus the rewritten index.
      expect(afterEntries.map(entry => entry.path).sort()).toEqual(
        [INDEXED, OVERWRITTEN, ...UNTOUCHED, VIA_SYMLINK.resolved, LLMS].sort(),
      )
    },
    SCRIPT_RUN_TIMEOUT_MS,
  )
...
// #419 (review round 7): /publish-pr Phase 1 no longer asks the agent to re-derive the
// snapshot/stage/commit recipe from prose — it runs the shipped KB asset. This is the
// producer → published identity → consumer proof, on the REAL installed artifact and
...
describe('the shipped mirror-realign asset drives the real writer (#419, /publish-pr Phase 1)', () => {
  let tmp = ''
  ...
  it(
    'HALTs on an untracked file under an unsafe tree without running the writer; stashed, commits exactly the regeneration and names the overwritten hand-edit',
    () => {
      tmp = makeFixture()
      git(tmp, ['config', 'core.excludesFile', '/dev/null'])
      const env = isolatedHome(tmp)
      ...
      expect(run(tmp, env).status).toBe(0)
      git(tmp, ['add', '-A'])
      git(tmp, ['commit', '-q', '-m', 'converged'])
      const head = git(tmp, ['rev-parse', 'HEAD']).trim()

      // The dataset gains a file (its mirror does not exist yet); HEAD's mirror of the index
      // is drifted AND carries the contributor's uncommitted hand-edit; authored work is
      // staged (src) and unstaged (notes); a WIP draft sits untracked under the mirror tree.
      write(join(tmp, 'packages/knowledge-hub/dataset/.pair/knowledge/new-guide.md'), NEW_GUIDE)
      writeFileSync(join(tmp, '.pair/knowledge/index.md'), '# my hand edit\n')
      writeFileSync(join(tmp, 'src/authored.ts'), 'v2 staged\n')
      git(tmp, ['add', 'src/authored.ts'])
      writeFileSync(join(tmp, 'notes.md'), 'v2 unstaged\n')
      write(join(tmp, '.pair/knowledge/wip-draft.md'), '# wip\n')

      const halted = realignViaAsset(tmp, env, ['.pair/knowledge/**'])
      expect(halted.status).toBe(2)
      expect(halted.stderr).toContain('.pair/knowledge/wip-draft.md')
      expect(halted.stderr).toContain("git stash push -u -- '.pair/knowledge/wip-draft.md'")
      // The writer did not run: the new mirror was not created, the hand-edit is intact.
      expect(existsSync(join(tmp, '.pair/knowledge/new-guide.md'))).toBe(false)
      expect(readFileSync(join(tmp, '.pair/knowledge/index.md'), 'utf-8')).toBe('# my hand edit\n')
      expect(git(tmp, ['rev-parse', 'HEAD']).trim()).toBe(head)

      git(tmp, ['stash', 'push', '-u', '-q', '--', '.pair/knowledge/wip-draft.md'])
      const ok = realignViaAsset(tmp, env, ['.pair/knowledge/**'])
      expect(ok.stderr).toBe('')
      expect(ok.status).toBe(0)
      const sha = git(tmp, ['rev-parse', '--short', 'HEAD']).trim()
      const committed = git(tmp, ['show', '--name-only', '--format=', '-z', 'HEAD'])
        .split('\0')
        .filter(Boolean)
      const row =
        /^regenerated — commit ([0-9a-f]+), (\d+) file\(s\); overwrote uncommitted changes in: \.pair\/knowledge\/index\.md \(recover: git cat-file -p ([0-9a-f]{40}) > \.pair\/knowledge\/index\.md\)\n$/.exec(
          ok.stdout,
        )
      expect(row).not.toBeNull()
      expect(row?.[1]).toBe(sha)
      ...
      expect(committed.every(path => !path.startsWith('packages/'))).toBe(true)
      expect(readFileSync(join(tmp, '.pair/knowledge/index.md'), 'utf-8')).toBe(KB_INDEX)
      expect(readFileSync(join(tmp, '.pair/knowledge/new-guide.md'), 'utf-8')).toBe(NEW_GUIDE)
      expect(git(tmp, ['cat-file', '-p', row?.[3] ?? ''])).toBe('# my hand edit\n')
      expect(git(tmp, ['diff', '--cached', '--name-only']).trim()).toBe('src/authored.ts')
      expect(readFileSync(join(tmp, 'notes.md'), 'utf-8')).toBe('v2 unstaged\n')

      git(tmp, ['stash', 'pop', '-q'])
      expect(readFileSync(join(tmp, '.pair/knowledge/wip-draft.md'), 'utf-8')).toBe('# wip\n')
      // And the second run is the silent no-op: nothing written, nothing reported.
      const again = realignViaAsset(tmp, env, [])
      expect(again).toEqual({ status: 0, stdout: 'no-op\n', stderr: '' })
    },
    SCRIPT_RUN_TIMEOUT_MS,
  )
})
