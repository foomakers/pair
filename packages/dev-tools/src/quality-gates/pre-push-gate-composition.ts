 ...
 *   banned outright.
 * - `mirrors:regenerate` (→ `scripts/regenerate-mirrors.sh`, #419) rewrites every
 *   generated mirror tree (`.claude/**`, root `.pair/**`, `AGENTS.md`/`CLAUDE.md`,
 *   `.github/agents/**`) from the local dataset. It is the second step of the gate's
 *   OWN remedy (`MIRROR_REMEDY_SCRIPT` below), so it must EXIST in package.json and
 *   must never be REACHED from the gate — two different checks. No check mode by
 *   design (the mirror guards are the checker), so banned outright.
 *
...
const WRITE_MODE_FORMATTERS: readonly { readonly name: string; readonly pattern: RegExp }[] = [
  ...
  { name: 'mirrors:regenerate', pattern: /\bmirrors:regenerate\b/ },
  // The `.sh` is required: `regenerate-mirrors.test.ts` (the script's own test file) is
  // not an invocation of it.
  { name: 'regenerate-mirrors', pattern: /\bregenerate-mirrors\.sh\b/ },
]
